            ...
            try:
                ...
                if live_candle:
                    ...
                    if not chart_df.empty and "datetime" in chart_df.columns:
                        ...
                        if getattr(dts.dt, "tz", None) is None and live_dt.tzinfo is not None:
                            dts = dts.dt.tz_localize(live_dt.tz)
                        elif getattr(dts.dt, "tz", None) is not None and live_dt.tzinfo is not None:
                            dts = dts.dt.tz_convert(live_dt.tz)
                        chart_df["datetime"] = dts
                        matches = chart_df["datetime"] == live_dt
                        if matches.any():
                            idx = chart_df.index[matches][-1]
                            for k, v in live_row.items():
                                chart_df.loc[idx, k] = v
                        else:
