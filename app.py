    ...
    with st.expander("Get a fresh access token", expanded=False):
        ...
        if st.session_state.auth_url:
            ...
            if st.button("🔐 Open Auth Web", use_container_width=True):
                st.session_state["show_fyers_auth_web"] = True
                st.rerun()
            st.caption("The Auth Web page helps you extract the auth code from the FYERS redirect URL.")
...
def _render_auth_web():
    ...
    st.caption("After you authorize FYERS, this page reads the returned URL and extracts the auth code for you.")

    st.info(
        "1. Click **Open FYERS Login** below.\n"
        "2. Complete the FYERS login/authorization.\n"
        "3. When FYERS redirects back, copy the full URL from your browser and paste it below."
    )
    ...
    if login_url:
        st.markdown(
            ...
            'font-weight:600;margin-bottom:1rem;">🔗 Open FYERS Login</a>',
            unsafe_allow_html=True,
        )

    st.caption("If FYERS has already redirected you, paste the complete URL from the browser address bar below.")

    # If this Streamlit page itself receives ?auth_code=..., pick it up automatically.
    try:
        qp = st.query_params
        qp_auth = qp.get("auth_code")
        if qp_auth and not st.session_state.get("fyers_auth_code"):
            st.session_state["fyers_auth_code"] = qp_auth
    except Exception:
        pass

    callback_url = st.text_input(
        "Paste the FYERS callback URL",
        key="fyers_callback_url",
        placeholder="https://fyers.in/?auth_code=...&state=..."
    )
    ...
    if callback_url:
        ...
        try:
            ...
            if auth_code:
                ...
                st.success("Auth code found.")
                st.code(auth_code, language=None)
                st.text_input(
                    "Auth code — copy from here",
                    value=auth_code,
                    key="fyers_extracted_auth_code",
                )
                st.session_state["fyers_auth_code"] = auth_code
            else:
                st.warning("No `auth_code` was found in that URL. Paste the complete redirect URL.")
        except Exception as exc:
            st.error(f"Could not parse the callback URL: {exc}")

    if st.button("← Back to FYERS connection", use_container_width=True):
        st.session_state["show_fyers_auth_web"] = False
        st.rerun()
