...
if _callback_code:
    # Restore the credentials saved when the user created the FYERS login URL.
    # Do not depend on the original Streamlit session surviving the OAuth
    # redirect: the browser may return through a fresh Streamlit session.
    ...
    # Exchange the one-time auth code immediately. This is intentional: after
    # the user returns from FYERS, the terminal can reconnect automatically
    # without asking them to paste anything again.
    if not st.session_state.get("token") or st.session_state.get("auth_token_state") != str(_callback_state or ""):
        try:
    ...
    # Remove the one-time code/state from the visible URL after capturing them.
    try:
...
if st.session_state.show_auth_callback and st.session_state.callback_auth_code:
    ...
    st.success("Auth code received from FYERS. Your App ID, Secret ID and Redirect URL have been restored automatically.")
