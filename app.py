...
def live_dashboard():
        ...
        for kind, data in engine.drain_events():
            if kind == "log":
                ...
                st.error(data)
                rejection = {"time": pd.Timestamp.now(tz="Asia/Kolkata").strftime("%H:%M:%S"), "type": "ERROR / REJECTED", "message": str(data)}
                st.session_state.rejected_orders.append(rejection)
                st.session_state.rejected_orders = st.session_state.rejected_orders[-50:]
                st.session_state.entry_log.append({"time": pd.Timestamp.now(tz="Asia/Kolkata"), "level":"error", "message":str(data)})
                st.session_state.entry_log = st.session_state.entry_log[-100:]
                st.toast(str(data), icon="⛔")
            elif kind == "signal":
                st.success(
