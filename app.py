...
def _chart_marker_time(df, value):
    ...
    try:
        ...
        resolution = int(st.session_state.get("chart_resolution_minutes", 5) or 5)
        tolerance = pd.Timedelta(minutes=max(1, resolution) * 2)
    ...
    for trade_no, trade in enumerate(trades or []):
        ...
        if entry_time is not None:
            ...
            markers.append({
                ...
                "text": f"BUY {option_type} {float(trade.get('entry', 0)):.2f}",
                "size": 2,
            })
            marker_no += 1
...
def _execution_table(execution_events):
    ...
    preferred = [
        ...
        "strike", "quantity", "lots", "entry", "status",
    ]
...
def _merge_execution_ledger(events):
    """Merge engine executions into Streamlit session state immediately."""
    ledger = list(st.session_state.get("algo_execution_ledger", []) or [])
    seen = {_execution_event_key(x) for x in ledger if _execution_event_key(x) is not None}
    changed = False
    for event in list(events or []):
        ...
        if key is None or key in seen:
            continue
        item = dict(event)
        # Ensure every persisted event has a stable chart bucket. Older engine
        # events may only have entry_time, so derive it from the execution time.
...
<span class="chart-count">Candle closes {countdown}</span>
        ...
        # Execution markers are built from the local execution ledger, not just
        # `last_signal`. This keeps every CE/PE entry visible after multiple
        # signals and maps intrabar triggers onto the candle that actually fired.
        ...
        if not live_execution_markers and engine.last_signal and not nifty_df.empty:
            # Fallback marker for a triggered signal whose option/order path is
        ...
        markers = live_execution_markers
...
def live_dashboard():
        ...
        for kind, data in engine.drain_events():
            if kind == "log":
                ...
                st.success(
                    f"✅ EXECUTED • BUY {option_type} • {data.get('symbol','')} "
                    f"@ ₹{float(data.get('entry',0)):.2f} • {int(data.get('quantity',0))} qty"
                )
            elif kind == "order":
        ...
        with tabs[0]:
            ...
            if engine.strategy.cross_price is not None:
                levels.append({"price":engine.strategy.cross_price,"title":"Cross","color":"#f0b90b","style":2})
            live_execution_events = _all_live_execution_events(engine)
            markers = _live_execution_markers(
            ...
            render_chart(chart_df, "NIFTY • FYERS", ltp=tick.get("ltp") if tick else None, levels=levels, markers=markers, height=620)
        ...
        with tabs[1]:
            ...
            if local_exec.empty:
                ...
                st.dataframe(local_exec, use_container_width=True, hide_index=True)
                st.caption("Entries are shown here immediately when the algo selects and executes a CE/PE contract.")
