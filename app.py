...
def render_replay_page():
    ...
    if "replay_cursor" not in st.session_state:
        st.session_state.replay_cursor = 30

    max_cursor = max(1, len(df))
    st.session_state.replay_cursor = min(max(1, int(st.session_state.replay_cursor)), max_cursor)
    cursor = st.slider("Replay candle", 1, max_cursor, st.session_state.replay_cursor, key="replay_cursor_slider")
    ...
    with rc2:
        if st.button("▶ Next candle", use_container_width=True):
            st.session_state.replay_cursor = min(max_cursor, st.session_state.replay_cursor + 1)
            st.rerun()
    with rc3:
        if st.button("▶▶ Run replay", use_container_width=True):
            replay_slice = df.iloc[:st.session_state.replay_cursor].copy()
            result = _run_backtest(replay_slice, rp_points, rp_bars, rp_sl, rp_target)
            st.session_state.replay_result = result
            st.session_state.replay_cursor = min(max_cursor, st.session_state.replay_cursor + 1)
            st.rerun()
    ...
    if not visible.empty:
        ...
        render_chart(replay_plot, f"REPLAY • {symbol} • {tf}m", vwap=True, markers=markers, height=650)
        st.caption(
            f"Replay is at candle {st.session_state.replay_cursor}/{len(df)}. "
