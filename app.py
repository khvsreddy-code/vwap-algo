...
def _run_backtest(df, confirmation_points, confirmation_bars, sl_points=0.0, target_points=0.0):
    ...
    # Keep an entry event visible even when the replay currently ends on the
    # entry candle.  It must NOT be converted into a fake exit.
    open_events = []
    if open_trade is not None:
        open_events.append({**open_trade, "status": "OPEN"})
...
def _chart_marker_time(df, value):
    ...
    try:
        ...
        candle_ns = candle_times[valid].astype("int64")
        if target_ns < int(candle_ns.min()) or target_ns > int(candle_ns.max()):
            return None
        idx = (candle_ns - target_ns).abs().idxmin()
        return int(candle_ns.loc[idx] // 1_000_000_000)
    except Exception:
        return None
    ...
    def _replay_player():
        ...
        if not visible.empty:
            ...
            if not replay_events.empty:
                ...
                st.dataframe(
                    replay_events[cols].sort_values("entry_time"),
