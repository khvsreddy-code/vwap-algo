...
def _run_backtest(df, confirmation_points, confirmation_bars, sl_points=0.0, target_points=0.0, session_start="09:15", session_end="15:15"):
    ...
    for bar_no, (_, row) in enumerate(prepared.iterrows()):
        ...
        if signal and open_trade is None:
            ...
            # Replay uses the NIFTY underlying as a proxy for the selected
            # option premium.  The held instrument is ALWAYS a long option:
            #   bullish signal -> BUY CE (premium benefits from NIFTY rising)
            #   bearish signal -> BUY PE (premium benefits from NIFTY falling)
            # Therefore the proxy exit direction is different for CE vs PE.
            # Keep `side=BUY` for the actual option semantics and store the
            # underlying direction separately so SL/target cannot be inverted.
            option_type = "CE" if signal_side == "BUY" else "PE"
            underlying_direction = 1 if option_type == "CE" else -1
            open_trade = {
                "side": "BUY",
                ...
                "lots": 1,
                "price_basis": "UNDERLYING_PROXY",
                # Long CE proxy: SL below / target above.
                # Long PE proxy: SL above / target below.
                "sl": entry - float(sl_points) * underlying_direction if sl_points > 0 else None,
                "target": entry + float(target_points) * underlying_direction if target_points > 0 else None,
        ...
        if open_trade is not None:
            exit_price = None
            reason = None
            # Conservative assumption when both levels are touched.  Use the
            # underlying direction explicitly: PE target is BELOW entry, while
            # PE stop is ABOVE entry. This is the key distinction that prevents
            # a falling NIFTY candle from being mislabeled as a PE stop-loss.
            direction = int(open_trade.get("underlying_direction", 1))
            if direction == 1:  # BUY CE / bullish proxy
    ...
    if open_trade is not None:
        ...
        if open_trade.get("_entry_bar", -1) < len(prepared) - 1:
            ...
            pnl = (exit_price - open_trade["entry"]) * int(open_trade.get("underlying_direction", 1))
            trades.append({
...
def _trade_markers(df, trades, include_exits=True):
    ...
    Underlying signal mapping:
      BUY signal  -> BUY CE
      SELL signal -> BUY PE
    Exits are exits of the held long option. CE/PE entry labels keep their
    original style; exit labels are explicitly colored by TARGET vs SL.
    """
    markers = []
    marker_no = 0
    for trade_no, trade in enumerate(trades or []):
        ...
        if include_exits:
            ...
            if exit_time is not None:
                ...
                if reason == "TARGET":
                    exit_color = "#14b8a6"  # teal: favorable exit
                    exit_text = f"TARGET HIT • BUY {option_type} @ {float(trade.get('exit', 0)):.2f}"
                elif reason == "SL":
                    exit_color = "#f97316"  # subtle red-orange: protective exit
                    exit_text = f"STOP LOSS HIT • BUY {option_type} @ {float(trade.get('exit', 0)):.2f}"
                else:
                    exit_color = "#94a3b8"
                    exit_text = f"EXIT • BUY {option_type} @ {float(trade.get('exit', 0)):.2f}"
                markers.append({
...
def render_charts_page():
    ...
    def _charts_live():
        ...
        data_source = str(getattr(engine, "data_source", "WEBSOCKET"))
        live_ok = bool(
            engine.running
            and tick
            and tick_age is not None
            and tick_age < 5
            and (engine.ws_connected or data_source == "REST_FALLBACK")
        )
        if live_ok and data_source == "REST_FALLBACK":
            status = "LIVE • REST FALLBACK"
        else:
...
def live_dashboard():
        ...
        status_text = (
            "LIVE • REST FALLBACK" if live_ok and data_source == "REST_FALLBACK"
            else "LIVE" if live_ok
            else "RECONNECTING" if engine.running and getattr(engine, "market_data_reconnecting", False)
            else "CONNECTING" if engine.running
            else "STOPPED"
        )
