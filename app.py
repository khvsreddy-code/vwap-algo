...
# Rerun the small Streamlit page every second so the browser chart receives
# the newest tick/candle from the background FYERS websocket.
...
if engine:
    ...
    # Paper engine follows the live market-data tick. It is completely separate
    # from the FYERS order path and can never submit a real order.
    if st.session_state.paper_trader is None:
        st.session_state.paper_trader = PaperTrader()
    paper_trader = st.session_state.paper_trader
    tick = engine.last_tick
    if tick:
        paper_trader.mark(tick["ltp"])
    ...
    status_text = "LIVE" if live_ok else ("CONNECTING" if engine.running else "STOPPED")
    ...
    ps = paper_trader.snapshot(tick["ltp"] if tick else None)
    ...
    if paper_manual:
        ...
        with pa:
            if st.button("🟢 BUY PAPER", use_container_width=True, disabled=not bool(tick) or bool(ps["qty"])):
                ok,msg=paper_trader.open(signal_symbol,"BUY",paper_qty,tick["ltp"],paper_sl,paper_target)
                (st.success if ok else st.error)(msg)
                st.rerun()
        with pb:
            if st.button("🔴 SELL PAPER", use_container_width=True, disabled=not bool(tick) or bool(ps["qty"])):
                ok,msg=paper_trader.open(signal_symbol,"SELL",paper_qty,tick["ltp"],paper_sl,paper_target)
                (st.success if ok else st.error)(msg)
                st.rerun()
        with pc:
            if st.button("✕ CLOSE PAPER", use_container_width=True, disabled=not bool(ps["qty"])):
                ok,res=paper_trader.close(tick["ltp"] if tick else 0)
                (st.success if ok else st.error)(f"Closed • P&L ₹{res:,.2f}" if ok else res)
                st.rerun()
