...
def _build_day_recovery_rows(client, instruments, signal_symbol, selected_date):
    """Fetch one day's 1m history for known cloud symbols and normalize it.

    This is an end-of-day gap-recovery path. It never treats FYERS REST history
    as a live feed and never deletes/replaces rows already stored in Supabase.
    """
    rows_by_symbol = []
    total = len(instruments) + 1
    progress = st.progress(0.0, text="Preparing end-of-day recovery…")
    status = st.empty()
    ...
    if not nifty_meta:
        ...
        # Keep NIFTY first for a predictable progress display.
    ...
    for idx, meta in enumerate(instruments, start=1):
        ...
        status.info(f"Fetching {idx}/{len(instruments)} • {symbol}")
        try:
            df = client.history_for_date(symbol, "1", selected_date, oi_flag=is_option)
        except Exception as exc:
            # Individual option contracts can legitimately have no historical
            # candles. Do not abort the whole day's recovery for one symbol.
            rows_by_symbol.append({"symbol": symbol, "rows": [], "error": str(exc)})
            progress.progress(idx / len(instruments), text=f"Skipped {symbol} • {idx}/{len(instruments)}")
            continue

        if df.empty:
            rows_by_symbol.append({"symbol": symbol, "rows": [], "error": None})
            progress.progress(idx / len(instruments), text=f"No FYERS history • {idx}/{len(instruments)}")
            continue
        ...
        progress.progress(idx / len(instruments), text=f"Fetched {len(normalized):,} candles • {idx}/{len(instruments)}")
...
def _store_fetch_instruments(store):
    """Compatibility wrapper for deployments where an older cloud_data.py is still loaded.

    The EOD page must not fail merely because the Streamlit runtime has an older
    CloudMarketStore class. Prefer the native method, then fall back to the
    underlying Supabase client.
    """
    method = getattr(store, "fetch_instruments", None)
    if callable(method):
        return method()
    response = (
        store.client.table("instruments")
        .select("*")
        .order("symbol")
        .execute()
    )
...
def _recover_missing_day(store, client, signal_symbol, selected_date, scope):
    ...
    results = _build_day_recovery_rows(client, selected, signal_symbol, selected_date)
    all_rows = []
    errors = []
    no_data = 0
    for item in results:
...
def _render_data_center():
    ...
    if client is None:
        ...
        st.caption(
            "End-of-day recovery is additive: FYERS history is fetched for the selected day, "
            "existing (symbol, minute) rows are skipped, and only missing candles are inserted. "
            "Existing Supabase rows are never overwritten by this recovery operation."
        )
        if st.button("🔄 Complete day & save ONLY missing data", type="primary", width="stretch", key="recover_missing_day"):
            try:
                signal_symbol = getattr(live_engine, "signal_symbol", None) or os.getenv("FYERS_SIGNAL_SYMBOL", "NSE:NIFTY50-INDEX")
                with st.spinner("Fetching FYERS history and checking Supabase for missing candles…"):
                    result = _recover_missing_day(store, client, signal_symbol, selected_date, scope)
                st.session_state.cloud_new_rows = result.get("new_rows", pd.DataFrame())
                st.session_state.cloud_recovery_summary = result
                st.success(
                    f"Day recovery complete • fetched {result['fetched']:,} • "
                    f"inserted {result['inserted']:,} new rows • skipped {result['skipped']:,} already in Supabase."
                )
                if result["no_data_symbols"]:
                    st.info(f"{result['no_data_symbols']:,} symbols returned no historical candles. That is normal for some option contracts.")
                if result["errors"]:
                    st.warning(f"{len(result['errors']):,} symbols had recoverable history errors; the rest of the day was still saved.")
            except Exception as exc:
                st.error(f"End-of-day recovery failed: {exc}")
...
<span class="chart-count">Candle closes {countdown}</span>
        ...
        if chart_timeframe == str(engine.resolution):
            ...
            # The previous version kept using the cached REST snapshot and only
            # overlaid a tick; after a socket reconnect/remount that could leave
            # the chart visually frozen even though the terminal LTP was moving.
        ...
        def option_chart():
            ...
            render_chart(ex, engine.selected_option["symbol"],
                         ...
                         vwap=False, levels=lev, height=900, max_candles=CHART_CANDLE_LIMIT, fit_content=False)

        if show_nifty and show_option and has_option:
            left, right = st.columns(2, gap="small")
            with left:
                st.markdown(f"#### NIFTY • VWAP • {chart_timeframe}m")
                render_chart(nifty_df, "NIFTY • FYERS", ltp=ltp, levels=levels, markers=markers, height=900, max_candles=CHART_CANDLE_LIMIT, fit_content=False)
            with right:
...
def live_dashboard():
        ...
        with tabs[0]:
            ...
            try:
                ...
                chart_df = _merge_chart_history(cache.get(chart_key), chart_df)
            except Exception as exc:
            ...
            render_chart(chart_df, "NIFTY • FYERS", ltp=tick.get("ltp") if tick else None, levels=levels, markers=markers, height=620, max_candles=CHART_CANDLE_LIMIT, fit_content=False)
