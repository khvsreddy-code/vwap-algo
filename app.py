...
from v9_4_55_live_chart import render as render_chart
...
<span class="chart-count">Candle closes {countdown}</span>
        ...
        def option_chart():
            ...
            render_chart(ex, engine.selected_option["symbol"],
                         ...
                         vwap=False, levels=lev, height=760, max_candles=180, fit_content=False)

        if show_nifty and show_option and has_option:
            left, right = st.columns(2, gap="small")
            with left:
                st.markdown(f"#### NIFTY • VWAP • {chart_timeframe}m")
                render_chart(nifty_df, "NIFTY • FYERS", ltp=ltp, levels=levels, markers=markers, height=760, max_candles=180, fit_content=False)
            with right:
...
def live_dashboard():
        ...
        with tabs[0]:
            ...
            render_chart(chart_df, "NIFTY • FYERS", ltp=tick.get("ltp") if tick else None, levels=levels, markers=markers, height=740, max_candles=180, fit_content=False)
                ...
                for pos in positions:
                    if pos.get("symbol") == engine.selected_option.get("symbol"):
                        try:
                            ...
                            pass
                render_chart(ex, engine.selected_option['symbol'], ltp=engine.last_execution_tick.get("ltp") if engine.last_execution_tick else engine.selected_option.get("ltp"), vwap=False, levels=lev, height=680, max_candles=180, fit_content=False)
