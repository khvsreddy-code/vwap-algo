...
if _callback_code:
    # This is the old working FYERS flow: FYERS redirects directly back to
    # the Streamlit app URL with ?s=ok&code=200&auth_code=...&state=...
    # Capture the code and render our own code page. Do not redirect to a
    # second /page=auth route and do not send the user to a FYERS page.
    ...
    if flow:
        ...
        # The callback flow is one-time. Clear the consumed state so the next
        # login attempt creates a fresh state instead of reusing a consumed one.
...
def _render_auth_web():
    ...
    # FYERS must redirect directly to the Streamlit app root. The callback
    # query string is then ?s=ok&code=200&auth_code=...&state=fyers_vwap.
...
with st.sidebar:
    ...
    with st.expander("Get a fresh access token", expanded=False):
        ...
        redirect_uri = st.text_input(
            ...
            help="Use the exact Streamlit app URL registered in your FYERS API app. It must match exactly.",
        )
        ...
        if st.session_state.auth_url:
            ...
            st.caption("Your App ID, Secret ID and Redirect URI are saved before this link is opened. FYERS returns directly to this Streamlit URL with ?page=auth&s=ok&code=200&auth_code=...&state=fyers_vwap.")
