...
if ui_theme == "Light":
    ...
    """
st.markdown("""<style>
/* v9.8 command-center UI: denser, calmer, and faster to scan. */
.block-container { padding-top: .55rem !important; }
.tradingcore-shell { display:flex; align-items:center; justify-content:space-between; gap:14px; padding:12px 16px; margin:0 0 10px; border:1px solid #1d2a38; border-radius:13px; background:linear-gradient(180deg,rgba(14,21,29,.97),rgba(9,14,20,.97)); box-shadow:0 8px 24px rgba(0,0,0,.12); }
.tradingcore-brand { display:flex; align-items:center; gap:11px; min-width:0; }
.tradingcore-logo { width:34px; height:34px; border-radius:10px; display:grid; place-items:center; background:#14202c; border:1px solid #26394b; font-size:18px; }
.tradingcore-name { font-size:17px; font-weight:800; letter-spacing:-.02em; color:#eef4fb; line-height:1.05; }
.tradingcore-sub { color:#8290a2; font-size:11px; margin-top:3px; }
.tradingcore-health { display:flex; align-items:center; gap:7px; flex-wrap:wrap; justify-content:flex-end; }
.tradingcore-pill { padding:5px 9px; border-radius:999px; border:1px solid #253545; background:#111a24; color:#9eabba; font-size:11px; font-weight:700; white-space:nowrap; }
.tradingcore-pill.good { color:#35d2a6; border-color:#245a4e; background:#0d211c; }
.tradingcore-pill.warn { color:#f1c363; border-color:#5a4925; background:#211b0d; }
.tradingcore-pill.bad { color:#ff747a; border-color:#5e3035; background:#241113; }
.tradingcore-page-title { margin:2px 0 8px; font-size:12px; color:#748296; text-transform:uppercase; letter-spacing:.09em; font-weight:800; }
/* Make Streamlit navigation feel like a terminal command bar. */
div[data-testid="stHorizontalBlock"]:has(button[key="nav_terminal"]) { margin-bottom:10px; }
section[data-testid="stSidebar"] .stButton button { min-height:38px; font-weight:700; }
@media (max-width: 900px) { .tradingcore-shell { align-items:flex-start; flex-direction:column; } .tradingcore-health { justify-content:flex-start; } }
