...
# ---------- browser session persistence for Auth Web ----------
# sessionStorage survives leaving Streamlit for FYERS and returning to the same
# browser tab, but is scoped to this app origin and disappears when that tab is
# closed. This prevents the App ID/Secret/Redirect fields from being wiped when
# the OAuth navigation creates a fresh Streamlit session.
try:
    _auth_store_component = st.components.v2.component(
        name="fyers_auth_session_store",
        html="<div style='display:none'></div>",
        js=r"""
export default function(component) {
  const {data, setStateValue} = component;
  const KEY = "fyers_vwap_auth_fields_v1";
  let stored = {};
  try { stored = JSON.parse(sessionStorage.getItem(KEY) || "{}"); } catch(e) {}

  const incoming = data || {};
  const hasIncoming = [incoming.app_id, incoming.secret_id, incoming.redirect_uri]
    .some(v => String(v || "").trim() !== "");

  // On the first render of a new Streamlit session, recover what the user
  // typed before navigating to FYERS. Otherwise keep the current Python values.
  if (!hasIncoming && (stored.app_id || stored.secret_id || stored.redirect_uri)) {
    setStateValue("app_id", stored.app_id || "");
    setStateValue("secret_id", stored.secret_id || "");
    setStateValue("redirect_uri", stored.redirect_uri || "");
  } else {
    const next = {
      app_id: String(incoming.app_id || stored.app_id || ""),
      secret_id: String(incoming.secret_id || stored.secret_id || ""),
      redirect_uri: String(incoming.redirect_uri || stored.redirect_uri || "")
    };
    try { sessionStorage.setItem(KEY, JSON.stringify(next)); } catch(e) {}
    setStateValue("app_id", next.app_id);
    setStateValue("secret_id", next.secret_id);
    setStateValue("redirect_uri", next.redirect_uri);
  }
}
""",
        isolate_styles=True,
...
try:
    _same_tab_auth_component = st.components.v2.component(
        name="fyers_same_tab_auth_link",
        html="<div id='auth-open'></div>",
        js=r"""
export default function(component) {
  const {data, parentElement} = component;
  const host = parentElement.querySelector("#auth-open");
  host.innerHTML = "";
  ...
  a.textContent = "🔐 Open Auth Web";
  a.style.cssText = "display:block;text-align:center;padding:.8rem 1rem;border-radius:.5rem;background:#2d2d38;color:white;text-decoration:none;font-weight:700;margin:.5rem 0 1rem;cursor:pointer";
...
# Recover auth fields from the browser tab before any auth widgets are created.
if _auth_store_component is not None:
    try:
        _auth_store_result = _auth_store_component(
            data={
                "app_id": st.session_state.get("auth_app_id", ""),
                "secret_id": st.session_state.get("auth_secret_id", ""),
                "redirect_uri": st.session_state.get("auth_redirect_uri", _DEFAULT_REDIRECT_URI),
            },
            default={"app_id": "", "secret_id": "", "redirect_uri": ""},
            key="fyers_auth_session_store",
            on_app_id_change=lambda: None,
            on_secret_id_change=lambda: None,
            on_redirect_uri_change=lambda: None,
        )
        # Only adopt non-empty browser values. The component writes them back
        # into sessionStorage whenever the Python values change.
        for _k in ("app_id", "secret_id", "redirect_uri"):
            _v = getattr(_auth_store_result, _k, None)
            if _v:
                setattr(st.session_state, "auth_" + _k, _v)
    except Exception:
        pass
...
def _render_auth_web():
    ...
    app_id = st.text_input(
        "App ID",
        value=st.session_state.get("auth_app_id") or os.getenv("FYERS_APP_ID", ""),
        key="auth_page_app_id",
    )
    secret_id = st.text_input(
        "Secret ID",
        value=st.session_state.get("auth_secret_id") or os.getenv("FYERS_SECRET_ID", ""),
        type="password",
        key="auth_page_secret_id",
    )
    redirect_uri = st.text_input(
        "Redirect URI",
        value=st.session_state.get("auth_redirect_uri") or recommended_redirect,
    ...
    if _auth_store_component is not None:
        try:
            _auth_store_component(
                data={"app_id": app_id.strip(), "secret_id": secret_id.strip(), "redirect_uri": redirect_uri},
                key="fyers_auth_session_store_auth_page",
                on_app_id_change=lambda: None,
                on_secret_id_change=lambda: None,
                on_redirect_uri_change=lambda: None,
            )
    ...
    if login_url:
        ...
        if _same_tab_auth_component is not None:
            _same_tab_auth_component(
                data={"url": login_url},
                key="auth_open_web_button",
            )
        else:
            st.markdown(
                f'<a href="{safe_login_url}" target="_self" rel="noopener noreferrer" '
...
with st.sidebar:
    ...
    with st.expander("Get a fresh access token", expanded=False):
        ...
        if _auth_store_component is not None:
            try:
                _auth_store_component(
                    data={"app_id": app_id.strip(), "secret_id": secret_id.strip(), "redirect_uri": redirect_uri.strip()},
                    key="fyers_auth_session_store_terminal",
                    on_app_id_change=lambda: None,
                    on_secret_id_change=lambda: None,
                    on_redirect_uri_change=lambda: None,
                )
        ...
        if st.session_state.auth_url:
            ...
            if _same_tab_auth_component is not None:
                _same_tab_auth_component(
                    data={"url": st.session_state.auth_url},
                    key="terminal_auth_open_web_button",
                )
            else:
                st.markdown(
                    f'<a href="{safe_login_url}" target="_self" rel="noopener noreferrer" '
