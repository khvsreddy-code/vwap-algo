...
<span class="chart-count">Candle closes {countdown}</span>
        ...
        # The engine normally owns the history. If its background lifecycle
        # has not populated history yet, use the cached REST history as the
        # authoritative chart source so the chart never renders an empty
        # candle series merely because the websocket is still connecting.
        if (base_nifty is None or base_nifty.empty) and engine.signal_symbol:
            try:
                base_nifty = _load_chart_history(
                    client, engine.signal_symbol, chart_timeframe, 31
                )
        ...
        if show_nifty and show_option and has_option:
            ...
            if nifty_df is None or nifty_df.empty:
                history_error = st.session_state.get("chart_history_error", "")
                st.warning(
                    "FYERS returned no candle bars for this chart timeframe."
                    + (f"  History API: {history_error}" if history_error else "")
                )
