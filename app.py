...
for key, default in {
    ...
    "paper_trader": None, "entry_log": [], "callback_auth_code": "", "show_auth_callback": False, "rejected_orders": [],
...
def render_charts_page():
    ...
    def _charts_live():
        ...
        status = "LIVE" if live_ok else (getattr(engine, "ws_state", None) or ("CONNECTING" if engine.running else "STOPPED"))
...
<span class="chart-count">Candle closes {countdown}</span>
        ...
        def option_chart():
            ...
            if ex is None or ex.empty:
                try:
                    ex = client.history(engine.selected_option["symbol"], engine.resolution, 3)
                except Exception:
                    ex = pd.DataFrame()
            lev = []
...
if connect or st.session_state.pop("do_connect", False):
    try:
        # Never leave an old engine/websocket running when reconnecting.
...
def live_dashboard():
        ...
        for kind, data in engine.drain_events():
            if kind == "log":
                ...
                # A market-data socket disconnect is a recoverable connection
                # event, not an order rejection. Keep it out of the broker
                # rejection panel and show a calm reconnecting warning.
                text = str(data)
                if "Market data" in text or "WebSocket" in text or "socket" in text.lower():
                    st.warning(f"📡 {text} • reconnecting automatically")
                    level = "warning"
                    icon = "📡"
                else:
                    st.error(text)
                    rejection = {"time": pd.Timestamp.now(tz="Asia/Kolkata").strftime("%H:%M:%S"), "type": "ERROR / REJECTED", "message": text}
                    st.session_state.rejected_orders.append(rejection)
                    st.session_state.rejected_orders = st.session_state.rejected_orders[-50:]
                    level = "error"
                    icon = "⛔"
                st.session_state.entry_log.append({"time": pd.Timestamp.now(tz="Asia/Kolkata"), "level":level, "message":text})
                st.session_state.entry_log = st.session_state.entry_log[-100:]
        ...
        # Keep the latest broker rejection visible after processing new events.
        render_rejection_panel()
        ...
        live_ok = bool(engine.running and engine.ws_connected and tick and tick_age is not None and tick_age < 5)
        status_text = "LIVE" if live_ok else (getattr(engine, "ws_state", None) or ("CONNECTING" if engine.running else "STOPPED"))
        ...
        if engine.running and not live_ok:
            if engine.ws_connected:
                st.warning("FYERS socket is connected, but no fresh tick has arrived. Check the symbol, market hours and FYERS feed permissions.")
            else:
                st.info("Connecting to FYERS live market data…")
        ...
        with st.expander("🧪 Entry engine monitor", expanded=True):
            ...
            if st.session_state.entry_log:
                ...
                rows = []
                for x in reversed(st.session_state.entry_log[-50:]):
                    t = x["time"]
