...
if _callback_code:
    ...
    # Exchange immediately so "Back to Terminal" can reconnect without
    # making the user paste the one-time code again.
    app_id_cb = st.session_state.get("auth_app_id", "")
    secret_id_cb = st.session_state.get("auth_secret_id", "")
    redirect_uri_cb = st.session_state.get("auth_redirect_uri", "")
    try:
        if not app_id_cb or not secret_id_cb or not redirect_uri_cb:
            raise ValueError("Saved App ID / Secret ID / Redirect URI were not available for the callback.")
        new_token = FyersClient.exchange_auth_code(
            app_id_cb, secret_id_cb, redirect_uri_cb, st.session_state.callback_auth_code
        )
...
if st.session_state.show_auth_callback and st.session_state.callback_auth_code:
    ...
    if st.button("↩️ Back to Terminal", type="primary", use_container_width=True):
        ...
        # Remove the one-time FYERS callback URL only AFTER the callback screen
        # has been displayed. This prevents the callback from being replayed.
...
def _render_auth_web():
    ...
    st.caption("Use the bare Streamlit app URL as the Redirect URL. After login, FYERS sends ?s=ok&code=200&auth_code=...&state=... directly back to this page.")

    app_url = os.getenv("STREAMLIT_APP_URL", "https://vwap-algo-pej2nt7fjsxausdc9trgnk.streamlit.app")
    # IMPORTANT: use the bare Streamlit root as the FYERS Redirect URL.
    # FYERS appends ?s=ok&code=200&auth_code=...&state=... to this URL.
    # Do NOT register /?page=auth as the redirect URI.
    ...
    if st.session_state.get("auth_callback_error"):
        st.error(f"FYERS authorization returned an error: {st.session_state.auth_callback_error}")
        st.session_state.auth_callback_error = ""
    ...
    if not login_url:
        ...
        # redirect URI is this Streamlit page, FYERS returns directly to this Streamlit auth route.
        ...
        st.caption("FYERS returns to this same Streamlit URL. If FYERS requires login/PIN, that happens on FYERS; after success it returns directly here with the auth code.")

    if st.session_state.get("callback_auth_code"):
        st.markdown("### 3. Authorization result")
        if st.session_state.get("auth_token_ready"):
            st.success("✅ FYERS authorized and today's access token was generated automatically.")
        else:
            st.warning("FYERS returned the auth code, but the access token could not be generated yet.")
            if st.session_state.get("auth_callback_error"):
                st.error(st.session_state.auth_callback_error)
        st.code(st.session_state.callback_auth_code, language=None)
        st.caption("The auth code is temporary and one-time use. Keep it private.")
        ...
        st.info("After you finish login, this page will automatically show the auth result. No URL-bar searching is required.")

    if st.button("↩️ Back to Terminal", type="primary", use_container_width=True):
        # Everything needed to reconnect is already in session state:
        # App ID, Secret ID, Redirect URI and the freshly generated token.
        # Ask the terminal to connect immediately after returning.
        st.session_state.show_auth_callback = False
        st.session_state.page = "terminal"
        st.session_state.do_connect = bool(st.session_state.get("token"))
        # The old auth code/state is one-time; a future Open Auth Web click
        # must create a fresh state and save the current input values again.
        st.session_state.auth_state = ""
...
with st.sidebar:
    ...
    with st.expander("Get a fresh access token", expanded=False):
        ...
        # IMPORTANT: persist the values continuously, not only after a
        # separate "Create login link" button. Streamlit may create a fresh
        # session when the browser returns from the external auth flow.
        #
        # We create the short-lived state as soon as the user has entered the
        # required fields, then update the server-side flow record on every
        # rerun. Therefore, by the time the user clicks "Open Auth Web", the
        # exact values currently visible in the inputs are already saved.
        ...
        if app_id.strip() and secret_id.strip() and redirect_uri.strip():
            try:
                # Never regenerate a login URL after a callback has already
                # been received. A fresh URL is created only after the user
                # explicitly starts a new auth attempt.
                if not st.session_state.get("callback_auth_code"):
                    if not st.session_state.get("auth_state"):
                        st.session_state.auth_state = "fyers_vwap_" + uuid.uuid4().hex
        ...
        if st.session_state.auth_url:
            ...
            st.markdown(
                f'<a href="{html.escape(st.session_state.auth_url, quote=True)}" target="_self" rel="noopener" '
                'style="display:block;text-align:center;padding:0.65rem 1rem;'
                'border-radius:0.5rem;background:#2d2d38;color:white;text-decoration:none;'
                'font-weight:700;margin:0.35rem 0 0.5rem;">🔐 Open Auth Web</a>',
                unsafe_allow_html=True,
            )
            st.caption("Your App ID, Secret ID and Redirect URI are saved before this link is opened. FYERS returns to the same Streamlit app root with ?s=ok&code=200&auth_code=...&state=....")
