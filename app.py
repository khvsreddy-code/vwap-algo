...
def _historical_signal_markers(
    ...
    for _, row in prepared.iterrows():
        ...
        markers.append({
            ...
            # Keep the visible chart label short and explicit.
            "text": f"BUY {option_type}",
            "size": 2,
...
<span class="chart-count">Candle closes {countdown}</span>
        ...
        def option_chart():
            ...
            lev = []
            option_events = [
                e for e in _all_live_execution_events(engine)
                if isinstance(e, dict) and e.get("symbol") == symbol
            ]
            option_markers = _live_execution_markers(ex, option_events)
            ...
            render_chart(ex, engine.selected_option["symbol"],
                         ...
                         vwap=False, levels=lev, markers=option_markers,
                         height=900, max_candles=180, fit_content=False)
