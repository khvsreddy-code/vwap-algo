...
def _render_data_center():
    ...
    if client is None:
        ...
        start_recovery = st.button(
            "🔄 Complete day & save ONLY missing data",
            type="primary",
            width="stretch",
        ...
        def _recovery_status_fragment():
            ...
            if current.get("done"):
                ...
                if summary:
                    st.success(
                        f"Day recovery complete • fetched {summary['fetched']:,} • "
                        f"inserted {summary['inserted']:,} new rows • "
                        f"skipped {summary['skipped']:,} already in Supabase."
                    )
...
def live_dashboard():
        ...
        def _terminal_chart_live():
            ...
            render_chart(chart_df, "NIFTY • FYERS", ltp=engine.last_tick.get("ltp") if engine.last_tick else None, levels=levels, markers=markers, height=620, max_candles=CHART_CANDLE_LIMIT, fit_content=False)
