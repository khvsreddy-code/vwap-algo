...
def _with_live_tick(df, tick, resolution):
    ...
    If REST history is temporarily empty, seed the current live candle so a
    newly-selected premium contract does not appear blank while its history
    request catches up.
    """
...
def _run_backtest(df, confirmation_points, confirmation_bars, sl_points=0.0, target_points=0.0,
                  session_start="09:15", session_end="15:15", session_flat_time="15:25",
                  quantity=1, estimated_cost_per_trade=0.0):
    """Run the VWAP confirmation state machine over completed candles.

    Results are based on the signal instrument's OHLC, not option premium OHLC.
    Positions are flattened before the next session, so an absent end-of-day
    stop/target cannot accidentally turn into an overnight result.
    """
    ...
    flat_minute = _hhmm_minute(session_flat_time, 15 * 60 + 25)
    if end_minute < start_minute:
        raise ValueError("Algo end time must be at or after the start time.")
    if flat_minute < end_minute:
        raise ValueError("Replay flat-at time must be at or after the entry end time.")
    quantity = max(1, int(quantity))
    estimated_cost_per_trade = max(0.0, float(estimated_cost_per_trade))
    ...
    def close_trade(trade, price, timestamp, reason):
        pnl_points = (float(price) - trade["entry"]) * (1 if trade["side"] == "BUY" else -1)
        return {
            **trade,
            "exit": float(price),
            "exit_time": timestamp,
            ...
            "gross_pnl_rupees": float(pnl_points * trade["quantity"]),
            "net_pnl_rupees": float(pnl_points * trade["quantity"] - estimated_cost_per_trade),
            "estimated_cost_rupees": estimated_cost_per_trade,
            "status": "CLOSED",
        }
    ...
    for bar_no, (_, row) in enumerate(prepared.iterrows()):
        ...
        # Historical feeds can skip the final candle. Close at the last known
        # price of the prior session instead of allowing an unmodelled overnight
        # position to be evaluated against tomorrow's opening range.
        if open_trade is not None and previous_date is not None and session_date != previous_date and previous_row is not None:
            trades.append(close_trade(open_trade, previous_row["close"], _ist_timestamp(previous_row["datetime"]), "SESSION_CLOSE"))
            open_trade = None
            strat.reset_trade()
        ...
        if open_trade is not None:
            ...
            # Do not assume overnight risk. A stop/target can exit earlier;
            # otherwise flat at the configured session close using its close.
            if minute_of_day >= flat_minute:
                exit_price, reason = float(row["close"]), "SESSION_CLOSE"
            # Conservative assumption when both levels are touched.
            elif open_trade["side"] == "BUY":
            ...
            if exit_price is not None:
                trades.append(close_trade(open_trade, exit_price, ts, reason))
                open_trade = None
                strat.reset_trade()
        previous_row = row
    ...
    if open_trade is not None:
        ...
        if open_trade.get("_entry_bar", -1) < len(prepared) - 1:
            ...
            trades.append(close_trade(open_trade, exit_price, last_ts, "END"))
            open_trade = None
    ...
    gross_pnl_rupees = float(sum(x.get("gross_pnl_rupees", x["pnl"] * x["quantity"]) for x in trades))
    net_pnl_rupees = float(sum(x.get("net_pnl_rupees", x["pnl"] * x["quantity"]) for x in trades))
    equity_curve = pd.Series([x["pnl"] for x in trades], dtype=float).cumsum()
    max_drawdown_points = float((equity_curve.cummax() - equity_curve).max()) if not equity_curve.empty else 0.0
    gross_profit = float(sum(x["pnl"] for x in trades if x["pnl"] > 0))
    gross_loss = abs(float(sum(x["pnl"] for x in trades if x["pnl"] < 0)))
    ...
    return {
        ...
        "gross_pnl_rupees": gross_pnl_rupees, "net_pnl_rupees": net_pnl_rupees,
        "max_drawdown_points": max_drawdown_points, "profit_factor": profit_factor,
...
def _trade_markers(df, trades, include_exits=True):
    ...
    for trade_no, trade in enumerate(trades or []):
        ...
        option_type = str(trade.get("option_type") or ("CE" if side == "BUY" else "PE")).upper()
...
def render_replay_page():
    ...
    r1, r2 = st.columns(2)
    with r1:
        rp_flat_time = st.time_input("Flat all positions at", value=dt_time(15, 25), key="rp_flat_time",
                                     help="Closes any remaining signal-instrument position at this candle's close. Prevents unmodelled overnight risk.")
    with r2:
        rp_cost = st.number_input("Estimated cost per trade (₹)", 0.0, 100000.0, 0.0, 1.0, key="rp_cost",
                                  help="Applied once to every completed trade for a conservative net-rupee estimate.")
    st.caption("Backtest P&L is based on NIFTY signal-instrument points, not option-premium fills. Net ₹ is a sizing and cost estimate, not a broker statement.")
    ...
    if result:
        ...
        e,f,g = st.columns(3)
        e.metric("Profit factor", "—" if result["profit_factor"] == float("inf") else f"{result['profit_factor']:.2f}")
        f.metric("Max drawdown", f"{result['max_drawdown_points']:.2f} pts")
        g.metric("Estimated net P&L", f"₹{result['net_pnl_rupees']:,.0f}")
    ...
    def _replay_player():
        ...
        if not visible.empty:
            ...
            qm1.metric("Profit Factor", "—" if sim.get("profit_factor") == float("inf") else f"{float(sim.get('profit_factor', 0.0)):.2f}")
            qm2.metric("Max Drawdown", f"{float(sim.get('max_drawdown_points', 0.0)):.2f} pts")
            qm3.metric("Estimated Net", f"₹{float(sim.get('net_pnl_rupees', 0.0)):,.0f}")
...
<span class="chart-count">Candle closes {countdown}</span>
        ...
        if st.button("↻ Reload historical chart", key="reload_live_chart_history"):
            cache = st.session_state.setdefault("_chart_history_cache", {})
            cache.pop((str(engine.signal_symbol), str(chart_timeframe), 31), None)
            ...
            if chart_timeframe == str(engine.resolution):
                ...
                if base_nifty is None or base_nifty.empty:
                    base_nifty = _load_chart_history(
                        client, engine.signal_symbol, chart_timeframe, 31
                    )
            else:
                base_nifty = _load_chart_history(
                    client, engine.signal_symbol, chart_timeframe, 31
        ...
        historical_markers = _historical_signal_markers(
            hist_marker_df,
        ...
        ) if not hist_marker_df.empty else []
        ...
        def option_chart():
            ...
            if st.session_state.get("option_chart_history_key") != option_key:
                if chart_timeframe == str(engine.resolution):
                    ...
                    if ex.empty:
                        ex = _load_chart_history(client, symbol, chart_timeframe, 31)
                else:
                    ex = _load_chart_history(client, symbol, chart_timeframe, 31)
            ...
            render_chart(ex, engine.selected_option["symbol"],
                         ...
                         vwap=False, levels=lev, height=900, max_candles=180, fit_content=False)

        if show_nifty and show_option and has_option:
            left, right = st.columns(2, gap="small")
            with left:
                st.markdown(f"#### NIFTY • VWAP • {chart_timeframe}m")
                render_chart(nifty_df, "NIFTY • FYERS", ltp=ltp, levels=levels, markers=markers, height=900, max_candles=180, fit_content=False)
            with right:
    ...
    try:
        ...
        try:
            ...
            if callable(start_order_ws):
                ...
                engine._emit_error_once(
                    "Order WebSocket helper unavailable; market-data feed remains active.",
                    key=("order_ws_helper_missing",),
                    cooldown=300,
                )
        except Exception as exc:
            engine._emit_error_once(
...
def live_dashboard():
        ...
        with tabs[0]:
            ...
            historical_markers = _historical_signal_markers(
                ...
                session_start=_hhmm_from_minute(engine.session_start_minute),
                session_end=_hhmm_from_minute(engine.session_end_minute),
            ) if not historical_df.empty else []
            live_execution_markers = _live_execution_markers(
            ...
            render_chart(chart_df, "NIFTY • FYERS", ltp=tick.get("ltp") if tick else None, levels=levels, markers=markers, height=620, max_candles=180, fit_content=False)
                ...
                for pos in positions:
                    if pos.get("symbol") == engine.selected_option.get("symbol"):
                        try:
                            ...
                            pass
                render_chart(ex, engine.selected_option['symbol'], ltp=engine.last_execution_tick.get("ltp") if engine.last_execution_tick else engine.selected_option.get("ltp"), vwap=False, levels=lev, height=500, max_candles=180, fit_content=False)
