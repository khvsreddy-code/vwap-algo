...
def _run_backtest(df, confirmation_points, confirmation_bars, sl_points=0.0, target_points=0.0, session_start="09:15", session_end="15:15"):
    ...
    for bar_no, (_, row) in enumerate(prepared.iterrows()):
        ...
        if signal and open_trade is None:
            ...
            # option premium.  A bullish VWAP signal (BUY CE) benefits when
            # NIFTY rises, while a bearish VWAP signal (BUY PE) benefits when
            # NIFTY falls.  Keep the displayed trade as BUY CE/BUY PE, but use
            # the corresponding underlying direction for SL/target math.
