...
<span class="chart-count">Candle closes {countdown}</span>
        ...
        if show_nifty and show_option and has_option:
            ...
            with left:
                ...
                render_chart(engine.display_history(), "NIFTY • FYERS", ltp=ltp, levels=levels, markers=markers, height=680)
            with right:
                st.markdown(f"#### OPTION • {engine.selected_option['symbol']}")
                option_chart()
        elif show_nifty:
            st.markdown("#### NIFTY • VWAP")
            render_chart(engine.display_history(), "NIFTY • FYERS", ltp=ltp, levels=levels, markers=markers, height=760)
