...
for key, default in {
    ...
    "auth_redirect_uri": os.getenv("FYERS_REDIRECT_URI", "https://vwap-algo-pej2nt7fjsxausdc9trgnk.streamlit.app/?page=terminal"),
...
if _callback_code:
    # FYERS returns directly to the Terminal URL. The one-time auth_code is
    # captured from the query string, exchanged immediately, then removed.
    ...
    try:
        ...
        st.session_state.auth_success_message = "FYERS authorization successful. Connecting to the Terminal…"
...
with st.sidebar:
    ...
    with st.expander("Get a fresh access token", expanded=False):
        st.info("Use this when your token has expired or you see FYERS -16. FYERS returns directly to the Terminal and the auth code is handled automatically.")
        ...
        redirect_uri = st.text_input(
            ...
            help="Use this Terminal URL as the Redirect URI registered in your FYERS API app. It must match exactly.",
        )
        ...
        if st.session_state.auth_url:
            ...
            st.caption("FYERS returns directly to the Terminal. The auth_code in the redirect URL is captured and exchanged automatically.")
