...
with st.sidebar:
    ...
    confirmation_points = st.number_input("Move after VWAP cross", 1.0, 100.0, 10.0, 0.5)
...
def _load_chart_history(client, symbol, resolution, days=31):
    ...
    for trade_no, trade in enumerate(trades or []):
        ...
        if entry_time is not None:
            ...
            markers.append({
                ...
                "text": f"BUY {option_type} {float(trade.get('entry', trade.get('trigger_price', 0))):.2f}",
                "size": 2,
            })
            marker_no += 1
...
def _persistent_execution_levels(execution_events, max_events=2):
...
<span class="chart-count">Candle closes {countdown}</span>
        ...
        def option_chart():
            ...
            render_chart(ex, engine.selected_option["symbol"],
                         ...
                         vwap=False, levels=lev, height=900, max_candles=None, fit_content=False)

        if show_nifty and show_option and has_option:
            left, right = st.columns(2, gap="small")
            with left:
                st.markdown(f"#### NIFTY • VWAP • {chart_timeframe}m")
                render_chart(nifty_df, "NIFTY • FYERS", ltp=ltp, levels=levels, markers=markers, height=900, max_candles=None, fit_content=False)
            with right:
        ...
        with st.expander("💼 Portfolio & Open Positions", expanded=False):
            ...
            if local_exec.empty:
                ...
                st.dataframe(local_exec, use_container_width=True, hide_index=True)
...
def live_dashboard():
        ...
        for kind, data in engine.drain_events():
            if kind == "log":
                ...
                status = str(data.get("status") or "TRIGGERED").upper()
                verb = "BUY"
                st.success(
                    f"🎯 {status} • {verb} {option_type} • {data.get('symbol','') or 'option pending'} "
                    f"@ ₹{float(data.get('entry',0) or 0):.2f} • {int(data.get('quantity',0) or 0)} qty"
                )
            elif kind == "order":
        ...
        with tabs[0]:
            ...
            render_chart(chart_df, "NIFTY • FYERS", ltp=tick.get("ltp") if tick else None, levels=levels, markers=markers, height=620, max_candles=None, fit_content=False)
                ...
                for pos in positions:
                    if pos.get("symbol") == engine.selected_option.get("symbol"):
                        try:
                            ...
                            pass
                render_chart(ex, engine.selected_option['symbol'], ltp=engine.last_execution_tick.get("ltp") if engine.last_execution_tick else engine.selected_option.get("ltp"), vwap=False, levels=lev, height=500, max_candles=None, fit_content=False)
