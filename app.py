...
def _run_backtest(df, confirmation_points, confirmation_bars, sl_points=0.0, target_points=0.0):
    ...
    for bar_no, (_, row) in enumerate(prepared.iterrows()):
        ...
        if signal and open_trade is None:
            ...
            # The signal is generated from the candle whose high/low reached the
            # confirmation level. OHLC data does not tell us whether the SL or
            # target was touched before or after that intrabar entry. Never
            # "fill" the exit on the same candle: exits become eligible starting
            # with the next completed candle. This keeps the backtest honest and
            # prevents BUY/SELL + EXIT from being stacked on one candle.
            continue
    ...
    # Never manufacture an exit on the same candle as the entry.  This matters
    # especially in replay, where the visible dataframe may end exactly on the
    # entry candle.  Leave that position open until a later candle is available.
    if open_trade is not None and open_trade.get("_entry_bar", -1) < len(prepared) - 1:
...
def _chart_marker_time(df, value):
    """Return the exact Unix-second timestamp of the nearest candle.

    Both the candle payload and marker payload are converted through the same
    UTC epoch path.  This avoids pandas' integer-timestamp unit ambiguity and
    guarantees that a marker uses a timestamp that actually exists in the
    candle series.
    """
    ...
    try:
        ...
        # Work entirely in integer Unix seconds.  Never pass a pandas integer
        # through pd.Timestamp(), whose default unit is nanoseconds.
        candle_epoch = candle_times[valid].astype("int64") // 1_000_000_000
        target_epoch = int(target.value // 1_000_000_000)

        deltas = (candle_epoch - target_epoch).abs()
        idx = deltas.idxmin()
        nearest_epoch = int(candle_epoch.loc[idx])

        # A trade should belong to this historical dataset.  Do not pin an
        # out-of-range event to the first/last candle.
        if abs(nearest_epoch - target_epoch) > 5 * 60:
            return None
...
def _trade_markers(df, trades, include_exits=True):
    """Build chronologically positioned entry/exit markers.

    Every marker time comes directly from a real candle in *df*.  Each marker
    also receives a unique id so the browser component cannot collapse events
    while updating the marker set.
    """
    markers = []
    marker_no = 0
    ...
    for trade_no, trade in enumerate(trades or []):
        ...
        if include_exits:
            ...
            if exit_time is not None:
                ...
                markers.append({
                    "id": f"exit-{trade_no}-{marker_no}",
                    "time": int(exit_time),
                    "position": "aboveBar" if side == "BUY" else "belowBar",
                    "color": "#ffd166",
                    "shape": "circle",
                    "text": f"{reason} {float(trade.get('exit', 0)):.2f}",
                    "size": 2,
                })
                marker_no += 1

    # Lightweight Charts requires markers in ascending time order.
    # Keep deterministic ordering for multiple events on one candle.
...
def render_replay_page():
    ...
    if result:
        ...
        if result["trades"]:
            bt_markers = _trade_markers(df, result["trades"], include_exits=True)
            render_chart(
                df,
    ...
    def _replay_player():
        ...
        if not visible.empty:
            ...
            markers = _trade_markers(visible, sim["trades"], include_exits=True)
