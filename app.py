...
# v9.3.4.8 — stable FYERS callback route (no redirect loop)
...
for key, default in {
    ...
    "auth_redirect_uri": os.getenv("FYERS_REDIRECT_URI", "https://vwap-algo-pej2nt7fjsxausdc9trgnk.streamlit.app/?page=auth"),
    "auth_state": "", "auth_flow_created_at": 0.0,
    "auth_token_state": "", "auth_token_ready": False, "auth_callback_error": "", "auth_success_message": "",
}.items(): ss(key, default)

# ---------- FYERS auth callback ----------
# FYERS returns auth_code + state to the exact Redirect URI registered in the
# FYERS developer console.  The auth page is deliberately a normal Streamlit
# route: it never redirects back to itself, which avoids Streamlit Cloud
# redirect loops.
...
def _save_auth_flows(flows):
    try:
        ...
        # Session state is still sufficient when the filesystem is read-only.
        pass
...
if _callback_code:
    # Do this once, then remove the one-time query values.  Importantly, we do
    # NOT navigate/redirect here.  That is what prevents the auth callback loop.
    ...
    if flow:
        ...
        st.session_state.auth_redirect_uri = flow.get("redirect_uri", st.session_state.get("auth_redirect_uri", ""))
        st.session_state.auth_state = str(_callback_state or flow.get("state", ""))

    code = str(_callback_code).strip()
    st.session_state.callback_auth_code = code
    st.session_state.page = "auth"

    try:
        if not st.session_state.auth_app_id or not st.session_state.auth_secret_id or not st.session_state.auth_redirect_uri:
            raise ValueError("The saved FYERS App ID / Secret ID / Redirect URI could not be restored.")
        new_token = FyersClient.exchange_auth_code(
            st.session_state.auth_app_id,
            st.session_state.auth_secret_id,
            st.session_state.auth_redirect_uri,
            code,
        )
        st.session_state.token = new_token
        st.session_state.auth_token_state = str(_callback_state or "")
        st.session_state.auth_token_ready = True
        st.session_state.auth_callback_error = ""
        st.session_state.auth_success_message = "FYERS authorization successful. Today's access token is ready."
        st.session_state.connected = False
        st.session_state.do_connect = True
    except Exception as e:
        st.session_state.auth_token_ready = False
        st.session_state.auth_callback_error = f"Token exchange failed: {e}"

    # Remove only the OAuth callback values.  Keep page=auth so the user lands
    # on the friendly callback page instead of the terminal or the root URL.
    try:
...
# ---------- page routing ----------
# OAuth callback must be able to arrive at /?page=auth even when Streamlit
# creates a fresh session.  Never redirect an auth page back to itself.
page = st.session_state.get("page", st.query_params.get("page", "terminal"))
if page not in ("terminal", "charts", "auth"):
    page = "terminal"
if _callback_code or _callback_error:
    page = "auth"
...
def _render_auth_page():
    st.markdown("# 🔐 FYERS Authorization")
    st.caption("This page is only the callback page. FYERS sends the one-time auth code here after login.")

    if st.session_state.get("auth_callback_error"):
        st.error(st.session_state.auth_callback_error)
        st.session_state.auth_callback_error = ""

    if st.session_state.get("auth_token_ready"):
        st.success("✅ FYERS authorization complete — today's access token is ready.")
        st.info("Your App ID, Secret ID and Redirect URI are preserved for this browser session. Click Back to Terminal; you should not need to paste them again.")
    elif st.session_state.get("callback_auth_code"):
        st.success("✅ Auth code received from FYERS.")
    else:
        st.info("Waiting for the FYERS authorization callback. If you reached this page manually, go back to Terminal and click Open FYERS Login.")
    ...
    if st.session_state.get("callback_auth_code"):
        ...
        st.caption("One-time code. Keep it private. The app has already exchanged it automatically when possible.")

    if st.session_state.get("auth_success_message"):
        st.success(st.session_state.auth_success_message)
        st.session_state.auth_success_message = ""

    if st.button("↩ Back to Terminal", key="auth_back_terminal", type="primary", use_container_width=True):
        _go_terminal()
        st.rerun()

    st.caption("You can safely bookmark this page. It will not redirect in a loop.")
    st.stop()
...
with st.sidebar:
    ...
    with st.expander("Get a fresh access token", expanded=False):
        st.info("Use this when your token has expired or you see FYERS -16. FYERS sends the login callback to the built-in Auth page, where the code is captured and the token is created automatically.")
        ...
        app_id = st.text_input(
            "App ID",
            value=st.session_state.auth_app_id or os.getenv("FYERS_APP_ID", ""),
            key="auth_app_id_input",
        )
        secret_id = st.text_input(
            "1. Secret ID",
            value=st.session_state.auth_secret_id or os.getenv("FYERS_SECRET_ID", ""),
            type="password",
        ...
        redirect_uri = st.text_input(
            ...
            help="Use this Auth URL as the Redirect URI registered in your FYERS API app. It must match exactly.",
        )

        # Keep the latest values available even if Streamlit creates a fresh
        # browser session after the external FYERS login.
        ...
        if st.session_state.auth_url:
            st.markdown("**4. Open FYERS Login**")
            ...
            st.markdown(
                f'<a href="{safe_login_url}" target="_self" rel="noopener" '
                'style="display:block;text-align:center;background:#ff4b4b;color:white;'
                'padding:0.75rem 1rem;border-radius:0.5rem;text-decoration:none;'
                'font-weight:700;margin:0.5rem 0 1rem;">🔐 Open FYERS Login</a>',
                unsafe_allow_html=True,
            )
            st.caption("FYERS returns to the built-in Auth page. The auth_code is captured and exchanged automatically, without opening another Streamlit page.")
        ...
        auth_code = st.text_input(
            ...
            help="Fallback only: the code is normally captured automatically when FYERS returns to the Terminal.",
        )
