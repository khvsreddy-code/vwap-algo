...
def _chart_marker_time(df, value):
    """Map a trade timestamp to an exact candle timestamp used by the chart.

    Lightweight Charts anchors markers to candle times. Trade timestamps can
    carry timezone/precision differences after pandas operations, so using the
    raw timestamp can place a marker at the chart edge instead of on its
    candle. Snap only to the nearest candle inside the loaded range.
    """
    if df is None or df.empty or value is None:
        return None
    try:
        target = pd.Timestamp(value)
        times = pd.to_datetime(df["datetime"])
        if times.dt.tz is None:
            target = target.tz_localize("Asia/Kolkata") if target.tzinfo is None else target.tz_convert("Asia/Kolkata")
            times = times.dt.tz_localize("Asia/Kolkata")
        else:
            target = target.tz_localize(times.dt.tz) if target.tzinfo is None else target.tz_convert(times.dt.tz)
        # Find the nearest actual candle. Do not silently map an out-of-range
        # trade to the first/last candle.
        deltas = (times - target).abs()
        idx = deltas.idxmin()
        if pd.isna(idx):
            return None
        nearest = times.loc[idx]
        if abs(nearest - target) > pd.Timedelta(minutes=5):
            return None
        return int(nearest.timestamp())
    except Exception:
        return None


def _trade_markers(df, trades, include_exits=True):
    """Build markers whose times are guaranteed to belong to chart candles."""
    markers = []
        ...
        if include_exits:
            ...
            if exit_time is not None:
                ...
                markers.append({
                    ...
                    "color": "#ffd166",
                    "shape": "circle",
                    "text": f"EXIT {float(trade.get('exit', 0)):.2f}",
                })

    # Lightweight Charts expects markers ordered by time.
    markers.sort(key=lambda x: (int(x["time"]), str(x.get("text", ""))))
    return markers
