...
def _within_entry_window(value):
...
def _run_backtest(df, confirmation_points, confirmation_bars, sl_points=0.0, target_points=0.0):
    ...
    for bar_no, (_, row) in enumerate(prepared.iterrows()):
        ...
        # Discard any NEW signal outside 09:15–15:15 IST.
        if signal and open_trade is None and not _within_entry_window(ts):
            try:
                strat.reset_trade()
                strat._clear_setup()
            except Exception:
...
def render_replay_page():
    ...
    st.info(
        "Trading session rule: NEW entries are allowed only from 09:15 to 15:15 IST. "
        "Existing positions can still hit SL/target after 15:15."
    )
    ...
    def _replay_player():
        ...
        if not visible.empty:
            ...
            if not replay_events.empty:
                ...
                st.info("No entries were taken inside the 09:15–15:15 IST entry window.")
