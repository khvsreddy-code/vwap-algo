...
# half-initialized module can surface as KeyError for a module that is currently
# being re-imported. Retry the two local modules that have historically hit this
# during hot-reload/deployment churn.
try:
    from cloud_data import CloudMarketStore
except KeyError as exc:
    if str(exc).strip("'\"") != "cloud_data":
        raise
    sys.modules.pop("cloud_data", None)
    from cloud_data import CloudMarketStore
...
.premium-live-card { background:#ffffff; border-color:#c8ded8; } .premium-live-title { color:#17202a; } .premium-live-detail { color:#64748b; } .premium-live-icon { background:#edf8f5; color:#087f5b; border-color:#b7ddd1; }
...
.premium-live-card { background:#090909; border-color:#333; } .premium-live-title,.premium-live-detail,.premium-live-state { color:#fff; } .premium-live-icon { background:#151515; border-color:#444; color:#fff; }
...
.chart-dot.warn { background:#f0b84b; box-shadow:0 0 10px rgba(240,184,75,.35); }
.premium-live-card { display:flex; align-items:center; gap:12px; margin:8px 0 10px; padding:12px 14px; border:1px solid #234238; border-radius:11px; background:linear-gradient(90deg,rgba(21,58,50,.58),rgba(13,21,29,.94)); box-shadow:0 8px 24px rgba(0,0,0,.12); }
.premium-live-icon { min-width:42px; height:30px; display:flex; align-items:center; justify-content:center; border-radius:8px; background:#14382f; border:1px solid #2d806c; color:#5ee7c1; font-size:12px; font-weight:800; letter-spacing:.5px; }
.premium-live-copy { min-width:0; }
.premium-live-title { color:#eaf7f3; font-size:13px; font-weight:800; }
.premium-live-detail { color:#91a9a4; font-size:11px; margin-top:2px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.premium-live-state { margin-left:auto; color:#61d9bc; font-size:10px; font-weight:800; letter-spacing:.6px; }
...
if ui_theme == "Light":
    ...
    """
st.markdown("""<style>
/* TradingCore v9.8.2 — polished terminal shell. UI-only styling; no market-data logic. */
:root { --tc-bg:#070b10; --tc-panel:#0c131c; --tc-panel2:#101923; --tc-border:#203040; --tc-muted:#7e8da0; --tc-text:#e9f0f7; --tc-accent:#4da3ff; --tc-teal:#21c7a3; --tc-red:#ff5b61; --tc-amber:#ffc857; }
.block-container { padding-top:.42rem !important; padding-bottom:1.2rem !important; max-width: 1680px !important; }
[data-testid="stAppViewContainer"] { background: radial-gradient(circle at 75% -10%, rgba(46,112,170,.13), transparent 32%), linear-gradient(180deg,#070b10 0%,#080d13 100%) !important; }
[data-testid="stHeader"] { background:rgba(7,11,16,.72) !important; backdrop-filter:blur(14px); border-bottom:1px solid rgba(43,59,76,.45); }
/* Header */
.tradingcore-shell { position:relative; overflow:hidden; display:flex; align-items:center; justify-content:space-between; gap:18px; padding:14px 17px; margin:0 0 12px; border:1px solid rgba(55,76,96,.72); border-radius:16px; background:linear-gradient(135deg,rgba(17,27,38,.97),rgba(8,14,21,.98)); box-shadow:0 12px 34px rgba(0,0,0,.22), inset 0 1px 0 rgba(255,255,255,.025); }
.tradingcore-shell:before { content:""; position:absolute; inset:-80px auto auto 42%; width:300px; height:160px; background:radial-gradient(circle,rgba(55,153,255,.12),transparent 68%); pointer-events:none; }
.tradingcore-brand { display:flex; align-items:center; gap:12px; min-width:0; position:relative; z-index:1; }
.tradingcore-logo { width:38px; height:38px; border-radius:12px; display:grid; place-items:center; background:linear-gradient(145deg,#15283a,#0d1823); border:1px solid #2b4963; box-shadow:0 0 0 1px rgba(77,163,255,.06),0 6px 18px rgba(0,0,0,.2); font-size:18px; }
.tradingcore-name { font-size:18px; font-weight:850; letter-spacing:.055em; color:#f4f8fc; line-height:1.05; }
.tradingcore-sub { color:#8293a6; font-size:10px; margin-top:4px; letter-spacing:.035em; }
.tradingcore-health { display:flex; align-items:center; gap:7px; flex-wrap:wrap; justify-content:flex-end; position:relative; z-index:1; }
.tradingcore-pill { padding:6px 10px; border-radius:999px; border:1px solid #263747; background:rgba(14,24,34,.9); color:#a5b3c1; font-size:10px; font-weight:800; letter-spacing:.045em; white-space:nowrap; box-shadow:inset 0 1px 0 rgba(255,255,255,.025); }
.tradingcore-pill.good { color:#4ce0b7; border-color:#1f5d4e; background:rgba(12,38,31,.9); }
.tradingcore-pill.warn { color:#ffd16b; border-color:#614d22; background:rgba(39,31,12,.9); }
.tradingcore-pill.bad { color:#ff7d82; border-color:#633137; background:rgba(42,16,19,.9); }
/* Navigation */
div[data-testid="stHorizontalBlock"]:has(button[key="nav_terminal"]) { padding:4px; margin:0 0 12px; border:1px solid #1c2a38; border-radius:13px; background:rgba(10,17,25,.84); box-shadow:0 7px 22px rgba(0,0,0,.14); }
div[data-testid="stHorizontalBlock"]:has(button[key="nav_terminal"]) button { min-height:34px !important; border-radius:9px !important; border-color:transparent !important; background:transparent !important; color:#8494a6 !important; font-size:11px !important; font-weight:800 !important; letter-spacing:.025em; transition:all .16s ease; }
div[data-testid="stHorizontalBlock"]:has(button[key="nav_terminal"]) button:hover { background:#15202c !important; color:#e9f0f7 !important; border-color:#263849 !important; transform:none !important; }
div[data-testid="stHorizontalBlock"]:has(button[key="nav_terminal"]) button[kind="primary"] { background:linear-gradient(180deg,#1b3349,#14283a) !important; color:#f5f9fd !important; border-color:#2c506d !important; box-shadow:inset 0 1px 0 rgba(255,255,255,.05),0 3px 10px rgba(0,0,0,.18); }
/* Sidebar shell */
section[data-testid="stSidebar"] { border-right:1px solid #182633 !important; box-shadow:8px 0 30px rgba(0,0,0,.12); }
section[data-testid="stSidebar"] > div { background:linear-gradient(180deg,#0a1118 0%,#0b121a 55%,#091018 100%) !important; }
section[data-testid="stSidebar"] .block-container { padding:1.05rem .9rem 1.4rem !important; }
section[data-testid="stSidebar"] h2 { font-size:14px !important; letter-spacing:.035em; color:#edf3f8 !important; margin-top:.55rem !important; margin-bottom:.35rem !important; }
section[data-testid="stSidebar"] h3 { font-size:11px !important; text-transform:uppercase; letter-spacing:.08em; color:#7f91a4 !important; margin-top:.7rem !important; }
section[data-testid="stSidebar"] [data-testid="stCaptionContainer"] { color:#728397 !important; font-size:10px !important; line-height:1.45 !important; }
section[data-testid="stSidebar"] hr { border-color:#1d2b38 !important; margin:14px 0 !important; }
section[data-testid="stSidebar"] input, section[data-testid="stSidebar"] textarea, section[data-testid="stSidebar"] [data-baseweb="select"] > div, section[data-testid="stSidebar"] [data-baseweb="input"] > div { border-radius:9px !important; border:1px solid #20303e !important; background:#0d151e !important; box-shadow:inset 0 1px 0 rgba(255,255,255,.018); }
section[data-testid="stSidebar"] input:focus, section[data-testid="stSidebar"] textarea:focus { border-color:#315f80 !important; box-shadow:0 0 0 1px rgba(77,163,255,.15) !important; }
section[data-testid="stSidebar"] .stButton button { min-height:37px !important; border-radius:9px !important; font-size:11px !important; font-weight:800 !important; transition:transform .12s ease,box-shadow .12s ease,border-color .12s ease; }
section[data-testid="stSidebar"] .stButton button:hover { transform:translateY(-1px); }
section[data-testid="stSidebar"] .stButton button[kind="primary"] { background:linear-gradient(135deg,#ff565c,#e8444c) !important; border-color:#ff666c !important; box-shadow:0 7px 18px rgba(236,69,77,.18) !important; }
/* Main content cards / metrics */
[data-testid="stMetric"] { background:linear-gradient(145deg,rgba(16,26,36,.96),rgba(10,17,24,.96)) !important; border:1px solid #1f3040 !important; border-radius:13px !important; padding:12px 14px !important; box-shadow:0 7px 20px rgba(0,0,0,.12), inset 0 1px 0 rgba(255,255,255,.018); }
[data-testid="stMetricLabel"] { color:#78899c !important; font-size:10px !important; font-weight:800 !important; text-transform:uppercase; letter-spacing:.055em; }
[data-testid="stMetricValue"] { color:#edf4f9 !important; font-weight:750 !important; letter-spacing:-.025em; }
/* Common app cards */
.card, .stage-card, .tv-top, .chart-status, .mode-banner, .page-nav { border-color:#203141 !important; border-radius:13px !important; box-shadow:0 7px 22px rgba(0,0,0,.12), inset 0 1px 0 rgba(255,255,255,.018); }
/* Expanders */
div[data-testid="stExpander"] { border:1px solid #1e2e3d !important; border-radius:11px !important; background:rgba(11,18,26,.68) !important; }
div[data-testid="stExpander"] summary { color:#cbd6df !important; font-size:11px !important; font-weight:750 !important; }
/* Dataframes */
[data-testid="stDataFrame"] { border:1px solid #1e2e3d !important; border-radius:11px !important; overflow:hidden; }
/* Alerts */
div[data-testid="stAlert"] { border-radius:11px !important; border-width:1px !important; }
/* Keep chart canvas clean — no decorative overlays on its surface. */
@media (max-width: 1100px) { .tradingcore-shell { align-items:flex-start; flex-direction:column; } .tradingcore-health { justify-content:flex-start; } }
@media (max-width: 700px) { .tradingcore-shell { padding:12px; } .tradingcore-name { font-size:16px; } .tradingcore-pill { padding:5px 8px; } }
</style>""", unsafe_allow_html=True)

# ---------- compact global status header ----------
# This header is intentionally derived only from existing session/engine state;
# it does not start a new request, socket, or rerun loop.
_hdr_engine = st.session_state.get("engine")
_hdr_client = st.session_state.get("client")
if _hdr_engine is not None:
    try:
        _age = _hdr_engine.tick_age_seconds()
    except Exception:
        _age = None
    if getattr(_hdr_engine, "ws_connected", False) and (_age is None or _age <= 5):
        _feed_cls, _feed_text = "good", "● LIVE"
    elif getattr(_hdr_engine, "market_data_reconnecting", False):
        _feed_cls, _feed_text = "warn", "● RECOVERING"
    else:
        _feed_cls, _feed_text = "bad", "● FEED OFFLINE"
    try:
        _ltp = (_hdr_engine.last_tick or {}).get("ltp")
        _ltp_text = f"NIFTY {_ltp:,.2f}" if _ltp is not None else "NIFTY —"
    except Exception:
        _ltp_text = "NIFTY —"
    _cloud = str(getattr(_hdr_engine, "cloud_status", "DISABLED") or "DISABLED")
else:
    _feed_cls, _feed_text = "warn", "● NOT CONNECTED"
    _ltp_text = "NIFTY —"
    _cloud = "DISABLED"

_cloud_cls = "good" if _cloud == "LIVE" else ("warn" if "DEGRADED" in _cloud or "RETRY" in _cloud else "")
_ws_label = _feed_text.replace("● ", "")
st.markdown(f"""
<div class="tradingcore-shell">
  <div class="tradingcore-brand">
    <div class="tradingcore-logo">📈</div>
    <div><div class="tradingcore-name">TRADINGCORE</div><div class="tradingcore-sub">FYERS VWAP execution terminal · v{APP_VERSION}</div></div>
  </div>
  <div class="tradingcore-health">
    <span class="tradingcore-pill {_feed_cls}">{_feed_text}</span>
    <span class="tradingcore-pill">{_ltp_text}</span>
    <span class="tradingcore-pill {_cloud_cls}">CLOUD {_cloud}</span>
  </div>
</div>
""", unsafe_allow_html=True)
...
def _render_data_center():
    ...
    if client is None:
        ...
        st.caption(
            ...
            "only missing candles inside that session. NIFTY 1-minute history is also checked "
            "for elapsed-session gaps and repaired with targeted history requests. "
            "Valid in-session rows are preserved."
        )
        ...
        def _recovery_status_fragment():
            ...
            if current.get("done"):
                ...
                if summary:
                    # Normalize summaries created by older app versions. This is
                    # important because Streamlit preserves session_state across
                    # reruns, so an old summary can survive a code update.
                    summary = {
                        "fetched": int(summary.get("fetched", 0)),
                        "inserted": int(summary.get("inserted", 0)),
                        "skipped": int(summary.get("skipped", 0)),
                        "deleted_out_of_session": int(summary.get("deleted_out_of_session", 0)),
                        "no_data_symbols": int(summary.get("no_data_symbols", 0)),
...
def _chart_history_days(resolution, candles=CHART_CANDLE_LIMIT):
    ...
    return max(7, min(730, calendar_days))
    ...
    for trade_no, trade in enumerate(trades or []):
        ...
        if include_exits:
            ...
            if exit_time is not None:
                ...
                is_target = reason in {"TARGET", "TP", "TARGET_HIT", "TP_HIT"}
                is_sl = reason in {"SL", "STOP", "STOP_LOSS", "STOP LOSS", "SL_HIT", "STOP_LOSS_HIT"}
                exit_color = "#16b8a6" if is_target else ("#f07b4f" if is_sl else "#9aa4b2")
                exit_label = "TARGET HIT" if is_target else ("STOP LOSS HIT" if is_sl else reason)
                markers.append({
                    ...
                    "color": exit_color,
                    "shape": "arrowDown",
                    "text": f"{exit_label} • BUY {option_type} @ {float(trade.get('exit', 0)):.2f}",
                    "size": 2,
                })
                marker_no += 1
...
def render_charts_page():
    ...
    @st.fragment(run_every="1s")
    def _charts_live():
        # Merge engine-side execution events before drawing. A live CE/PE trigger
        # is produced on a background thread; doing this first keeps the premium
        # leg and its contract details in the same render as the trigger marker.
        try:
        ...
        if live_ok:
            status = "LIVE"
        elif engine.running and getattr(engine, "market_data_blocked", False):
            status = "FEED BLOCKED"
        elif engine.running and getattr(engine, "market_data_reconnecting", False):
            status = "RECONNECTING"
        elif engine.running:
            status = "CONNECTING"
        else:
...
<span class="chart-dot {'on' if live_ok else ('warn' if status == 'RECONNECTING' else 'off')}"></span>
<b>{status}</b>
<span>• NIFTY {f"{ltp:,.2f}" if ltp is not None else "—"}</span>
<span>• Tick age {f"{tick_age:.1f}s" if tick_age is not None else "—"}</span>
<span>• {'Auto-recovery active' if status == 'RECONNECTING' else 'FYERS WebSocket'}</span>
<span class="chart-count">Candle closes {countdown}</span>
</div>""",
            unsafe_allow_html=True,
        )
        st.caption("Market data: FYERS WebSocket → canonical candle engine → chart. REST History is used only for bootstrap/recovery.")
        ...
        # Show the premium leg immediately when a CE/PE trigger occurs. The
        # option-selection/order worker is intentionally asynchronous so the
        # market-data callback never blocks; the UI therefore has a short
        # selecting state instead of making it look like the premium leg
        # disappeared. Once selected, the same card becomes the live premium
        # summary and the option chart is rendered below.
        ledger_now = list(st.session_state.get("algo_execution_ledger", []) or [])
        latest_trigger = next(
            (e for e in reversed(ledger_now)
             if isinstance(e, dict) and str(e.get("status", "")).upper()
             not in {"REJECTED", "FAILED", "CANCELLED"}),
            None,
        )
        if latest_trigger and (latest_trigger.get("symbol") or str(latest_trigger.get("status", "")).upper() in {"TRIGGERED", "QUEUED", "OPTION_SELECTED", "EXECUTED", "TEST", "PAPER"}):
            opt_type = str(latest_trigger.get("option_type") or ("CE" if str(latest_trigger.get("signal_side", "BUY")).upper() == "BUY" else "PE")).upper()
            opt_symbol = latest_trigger.get("symbol")
            opt_entry = latest_trigger.get("entry")
            try:
                opt_entry_f = float(opt_entry) if opt_entry is not None else None
            except (TypeError, ValueError):
                opt_entry_f = None
            opt_status = str(latest_trigger.get("status") or "TRIGGERED").upper()
            if opt_symbol and opt_entry_f is not None:
                premium_title = f"BUY {opt_type} • PREMIUM ACTIVE"
                premium_detail = f"{opt_symbol}  •  Entry ₹{opt_entry_f:,.2f}"
            else:
                premium_title = f"BUY {opt_type} • SELECTING PREMIUM"
                premium_detail = f"FYERS is selecting the best contract in ₹{premium_min:.0f}–₹{premium_max:.0f}  •  {opt_status}"
            st.markdown(
                f"""<div class=\"premium-live-card\">
                    <div class=\"premium-live-icon\">{opt_type}</div>
                    <div class=\"premium-live-copy\">
                      <div class=\"premium-live-title\">{premium_title}</div>
                      <div class=\"premium-live-detail\">{premium_detail}</div>
                    </div>
                    <div class=\"premium-live-state\">{opt_status}</div>
                </div>""",
                unsafe_allow_html=True,
            )
        ...
        levels, markers = [], []
        # Only the CURRENT strategy setup is allowed to draw Cross/Trigger
        # levels. Do not replay old execution-ledger levels as if the engine is
        # still waiting on them; that was the source of the misleading stale
        # low trigger line after a failed cross/reclaim.
        ...
        if st.session_state.get("chart_history_key") != chart_key:
            # Reuse the engine's already-loaded FYERS history when the chart
            # timeframe matches the strategy timeframe. This removes a second
            # identical History REST bootstrap request and materially reduces
            # the chance of a 429 during Connect. Other chart timeframes use
            # one cached History request.
            if chart_timeframe == str(engine.resolution) and engine.history_df is not None and not engine.history_df.empty:
                base_nifty = engine.history_df.copy()
            else:
            ...
            # If the first fragment rendered before the asynchronous engine
            # history bootstrap completed, adopt that now instead of keeping an
            # empty cached chart forever.
            if (chart_timeframe == str(engine.resolution)
                    and (base_nifty is None or base_nifty.empty)
                    and engine.history_df is not None and not engine.history_df.empty):
                base_nifty = engine.history_df.copy()
                st.session_state.chart_history_nifty = base_nifty.copy()

        # The engine is the single live-data authority. It aggregates every FYERS
        # tick into the selected chart timeframe, so a Streamlit rerun never
        # manufactures a candle by overlaying one LTP onto a stale REST row.
        live_chart_df = engine.display_chart_live(chart_timeframe)
        if live_chart_df is not None and not live_chart_df.empty:
            nifty_df = _merge_chart_history(base_nifty, live_chart_df)
        else:
            nifty_df = base_nifty.copy() if base_nifty is not None else pd.DataFrame()
        ...
        def _terminal_chart_live():
            ...
            try:
                ...
                if chart_key not in cache:
                    raw_chart_df = client.history(
                        engine.signal_symbol, engine.resolution, chart_days
                    )
                    # The REST history is raw OHLCV. Prepare it once so the NIFTY
                    # chart receives the same session-reset VWAP used by the
                    # strategy. Without this, render_chart(vwap=True) has no
                    # VWAP column to draw even though the strategy itself has one.
                    if raw_chart_df is not None and not raw_chart_df.empty:
                        cache[chart_key] = VwapConfirmationEngine.prepare(raw_chart_df.copy())
                    else:
                        cache[chart_key] = pd.DataFrame()
                ...
                # Recover gracefully from a cache created by an older build
                # where raw history was stored without the derived VWAP column.
                if not chart_df.empty and "vwap" not in chart_df.columns:
                    chart_df = VwapConfirmationEngine.prepare(chart_df)
                    cache[chart_key] = chart_df.copy()
