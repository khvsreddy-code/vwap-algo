...
def live_dashboard():
        ...
        with tabs[0]:
            ...
            # The engine's market-data socket runs in a background thread. On
            # Streamlit Cloud the first script render can happen before that
            # thread has completed the REST history seed, leaving a perfectly
            # sized chart with zero candles forever if no subsequent rerun
            # occurs. Make the chart deterministic: if its source dataframe is
            # empty, synchronously seed history from FYERS before rendering.
            if chart_df is None or chart_df.empty:
                try:
                    seeded = engine.load_history(days=31)
                    chart_df = engine.display_history()
                    if (chart_df is None or chart_df.empty) and seeded is not None:
                        chart_df = seeded.copy()
                except Exception as exc:
                    st.warning(
                        f"Unable to load NIFTY candle history: {exc}",
                        icon="⚠️",
                    )
