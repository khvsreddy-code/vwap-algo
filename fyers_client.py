...
class FyersClient:
    ...
    def start_data_socket(
        ...
        # Newer FYERS SDK builds expose reconnect_retry. Keep compatibility
        # with older builds by falling back when the keyword is unavailable.
        try:
            sock = data_ws.FyersDataSocket(reconnect_retry=3, **kwargs)
        except TypeError:
            sock = data_ws.FyersDataSocket(**kwargs)
        sock.connect()
        return sock
