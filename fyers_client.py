...
class FyersClient:
    ...
    def history(self, symbol, resolution, days=31, oi_flag=False):
        ...
        for row in all_candles:
            ...
            if len(vals) < len(columns):
                vals = vals + [None] * (len(columns) - len(vals))
    ...
    def history_for_date(self, symbol, resolution, trading_date, oi_flag=False):
        """Fetch one exact IST trading day of intraday history.

        Used by the cloud end-of-day recovery path so it can fill gaps without
        refetching several days of history for every option contract. Empty
        results are returned as an empty DataFrame; that is normal for symbols
        that did not trade or whose historical option data is unavailable.
        """
        ...
        requests = [
            {
                ...
                "resolution": str(resolution),
                "date_format": "1",
                "range_from": day.date().isoformat(),
                "range_to": next_day.date().isoformat(),
            },
            {
                "symbol": symbol,
                "resolution": str(resolution),
                "date_format": "0",
                "range_from": str(int(day.tz_localize(IST).timestamp())),
                "range_to": str(int(next_day.tz_localize(IST).timestamp())),
            },
        ]
        ...
        candles = []
        for req in requests:
            # FYERS can rate-limit the large end-of-day recovery sweep. Retry
            # only rate-limit failures with exponential backoff; normal no-data
            # responses remain cheap and are treated as empty history.
            for attempt in range(5):
                try:
                    resp = self._check(self.rest.history(data=req), "History")
                    payload = resp.get("data", resp)
                    candidate = payload.get("candles", []) if isinstance(payload, dict) else resp.get("candles", [])
                    if candidate:
                        candles = candidate
                        break
                    last_error = None
            ...
            if candles:
                break
            # A small pacing gap prevents a 200-symbol recovery from becoming
            # a burst of back-to-back REST requests.
            time.sleep(0.12)

        if not candles:
            if last_error is not None:
                # No-data is expected for some option contracts; let the caller
                # classify it as a skipped symbol instead of failing the day.
        ...
        max_cols = max((len(row) for row in candles if isinstance(row, (list, tuple))), default=6)
        columns = ["timestamp", "open", "high", "low", "close", "volume"]
        if oi_flag and max_cols >= 7:
            columns.append("oi")
        normalized = []
        ...
        df = pd.DataFrame(normalized, columns=columns)
        df["datetime"] = pd.to_datetime(
            pd.to_numeric(df["timestamp"], errors="coerce"), unit="s", utc=True, errors="coerce"
        ).dt.tz_convert(IST)
        for col in ["open", "high", "low", "close", "volume"]:
            df[col] = pd.to_numeric(df[col], errors="coerce")
        if "oi" in df.columns:
        ...
        # Keep only the requested IST calendar day even if FYERS interprets
        # range_to as inclusive.
        df = df[(df["datetime"].dt.date >= day.date()) & (df["datetime"].dt.date < next_day.date())].copy()
        return df.drop(columns="timestamp", errors="ignore")
    ...
    def start_data_socket(
        ...
        The application owns socket reconnects. The SDK's automatic reconnect
        is intentionally disabled because some SDK builds can race their
        message-queue worker during reconnect/teardown. Subscriptions are
        restored by the on_connect callback on every new socket.
        ...
        def _connect():
            ...
            try:
                ...
                if on_connect:
                    on_connect(sock)
                # Keep the websocket alive until the SDK reports that its
                # lifecycle has ended. Some SDK builds return from connect()
                # asynchronously, so this also prevents the caller from
                # mistaking a normal async return for a dead socket.
                sock.keep_running()
            finally:
                lifecycle_done.set()

        def _close(msg=None):
            # IMPORTANT: with reconnect=True, FYERS may call on_close during an
            # internal reconnect cycle. Do NOT wake the outer socket owner here,
            # otherwise it creates a second FyersDataSocket while the first SDK
            # instance is still reconnecting. That race was producing the
            # repeated socket-closed / NoneType thread errors and eventually
            # leaving the chart with a stale last tick. The lifecycle event is
            # released only when keep_running() actually returns.
            if on_close:
                on_close(msg)

        # Do NOT enable the SDK's automatic reconnect here. Some FYERS SDK
        # builds tear down message_thread_stop_event while their internal
        # __process_message_queue thread is still running, producing:
        #   AttributeError: 'NoneType' object has no attribute 'is_set'
        # The application owns the outer reconnect loop instead. This gives us
        # one socket at a time and one owner for socket teardown.
        ...
        # Wait in short intervals so the socket owner can perform a clean
        # teardown when the engine watchdog marks the feed stale. Crucially,
        # close_connection() is called by this same socket-owner thread rather
        # than by the watchdog thread, avoiding the SDK queue-thread race.
        while not lifecycle_done.wait(1.0):
