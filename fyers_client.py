...
class FyersClient:
    ...
    def history(self, symbol, resolution, days=31):
        """Fetch robust historical candles for charts and strategy.

        The History API accepts calendar-date or epoch ranges.  For a live
        terminal, a broad request can occasionally return ``no_data`` even
        though recent individual sessions are available, so we retry recent
        sessions before giving the chart an empty dataframe.
        """
        ...
        def request_range(range_from, range_to, date_format):
            data = {
                "symbol": symbol,
                ...
                # Continuous-contract flag is useful for futures. For indices,
                # equities and options, use the non-continuous form.
                "cont_flag": "1" if "FUT" in symbol else "0",
            }
            response = self.rest.history(data=data)
            if not isinstance(response, dict):
                return []
        ...
        # Preferred request: the complete configured window.
        try:
            rows = request_range(
        ...
        # Reliable fallback: request recent calendar sessions separately.
        # This handles weekends/holidays and transient broad-range no_data.
        if not batches:
            lookback = min(max(int(days), 7), 14)
            for offset in range(lookback):
                day = (end_dt - timedelta(days=offset)).date()
                try:
                    rows = request_range(day.isoformat(), day.isoformat(), "1")
                    if rows:
                        batches.append(rows)
                except Exception as exc:
                    errors.append(exc)

        # Final epoch-range fallback for SDK/API variants.
        if not batches:
            try:
                rows = request_range(
        ...
        # Final invariant for both the strategy and chart: valid OHLC bars,
        # strictly ascending and unique timestamps.
        df = (
