...
class FyersClient:
    ...
    def history_range(self, symbol, resolution, start, end):
        """Fetch FYERS candles for an explicit inclusive date range."""
        start = pd.Timestamp(start).date()
        end = pd.Timestamp(end).date()
        if end < start:
            raise ValueError("History end date must be on or after the start date.")
        data = {
    ...
    def history(self, symbol, resolution, days=31):
        ...
        return self.history_range(symbol, resolution, start, end)
    ...
    def start_order_socket(self, on_orders=None, on_trades=None, on_positions=None, on_general=None):
        ...
        """
        if not order_ws:
            raise RuntimeError("Install fyers-apiv3 with order websocket support.")
        callbacks = dict(
