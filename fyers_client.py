...
class FyersClient:
    ...
    def history(self, symbol, resolution, days=31):
        """Fetch reliable intraday history for charts/strategy.

        FYERS accepts calendar-date ranges, but using an exact epoch window as a
        second attempt is useful for intraday/index history and avoids returning
        an apparently successful empty response on some SDK/API combinations.
        The chart layer must never silently receive duplicate/out-of-order bars.
        """
        end_dt = datetime.now(IST)
        start_dt = end_dt - timedelta(days=max(1, int(days)))

        # cont_flag=1 is specifically for continuous futures. Sending it for
        # an NSE index can produce an empty history response on some API/SDK
        # combinations, which is particularly bad for the first Streamlit
        # render because the chart then has no candles to draw.
        ...
        date_request = {
            ...
            "date_format": "1",
            "range_from": start_dt.date().isoformat(),
            "range_to": end_dt.date().isoformat(),
        }
        epoch_request = {
            "symbol": symbol,
            "resolution": str(resolution),
        ...
        if is_continuous_future:
            date_request["cont_flag"] = "1"
            epoch_request["cont_flag"] = "1"

        requests = [date_request, epoch_request]
