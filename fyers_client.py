...
class FyersClient:
    ...
    def history_for_date(self, symbol, resolution, trading_date, oi_flag=False):
        """Fetch the complete NSE cash/option session for one IST trading day.

        FYERS can return a partial intraday result for a broad day-range request,
        which is especially problematic during end-of-day recovery.  Recovery
        therefore uses several explicit IST session chunks and merges them.
        The persisted dataset is hard-bounded to the NSE session:
        ...
        # Short, explicit windows avoid a provider-side partial-day response.
        # Keep the final boundary at 15:30; the 15:29 candle is the final
        # one-minute market candle and a 15:30-start candle is outside session.
        chunk_starts = [
            session_start,
        ...
        for req in requests:
            ...
            # Small pacing gap between chunks protects the FYERS REST limit.
            time.sleep(0.12)
        ...
        # This is the critical guard: regardless of what FYERS returns for a
        # chunk, recovery can never save candles outside the actual NSE session.
        df = df[
