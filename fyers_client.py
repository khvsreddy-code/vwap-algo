...
class FyersClient:
    ...
    def history(self, symbol, resolution, days=10):
    ...
    def choose_option(self, underlying, side, premium_min, premium_max, premium_target,
        ...
        first = self.option_chain(underlying, strikecount=strikecount, greeks=True)
        data = first.get("data", {})
        ...
        chain_resp = self.option_chain(
            underlying, strikecount=strikecount, timestamp=expiry.get("expiry"), greeks=True
        )
    ...
    def start_data_socket(self, symbols, on_message, on_error=None, on_close=None, on_connect=None):
        ...
        sock = data_ws.FyersDataSocket(
            ...
            on_connect=_connect, on_close=on_close or (lambda msg: None),
            on_error=on_error or (lambda msg: None), on_message=on_message,
        )
        sock.connect()
        return sock
