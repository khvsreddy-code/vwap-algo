...
class CloudMarketStore:
    ...
    def delete_out_of_session_candles(
        ...
        """Delete all rows outside the canonical NSE session for one IST day.

        The database stores ``candle_start`` as timestamptz.  Build the session
        boundaries in IST, convert them explicitly to UTC, and use those
        absolute instants in every PostgREST filter.  A verification pass then
        removes any rows that a broad timestamp delete failed to affect.
        """
        ...
        ist = ZoneInfo(IST)
        session_start_ist = day.to_pydatetime().replace(tzinfo=ist, hour=9, minute=15, second=0, microsecond=0)
        session_end_ist = day.to_pydatetime().replace(tzinfo=ist, hour=15, minute=30, second=0, microsecond=0)
        day_start_ist = day.to_pydatetime().replace(tzinfo=ist, hour=0, minute=0, second=0, microsecond=0)
        day_end_ist = day_start_ist + pd.Timedelta(days=1).to_pytimedelta()

        # Explicit UTC boundaries avoid any ambiguity in PostgREST/Postgres
        # timestamp comparisons.  09:15 IST = 03:45 UTC and
        # 15:30 IST = 10:00 UTC on normal Indian-localized dates.
        day_start = day_start_ist.astimezone(timezone.utc)
        session_start = session_start_ist.astimezone(timezone.utc)
        session_end = session_end_ist.astimezone(timezone.utc)
        ...
        def _delete_range(start_utc, end_utc):
            query = self.client.table("market_candles_1m").delete(count="exact")
            query = _apply_scope(query)
            response = (
                query.gte("candle_start", start_utc.isoformat())
                     .lt("candle_start", end_utc.isoformat())
                     .execute()
            )
            return int(response.count or 0)

        # Fast path: two indexed range deletes.
        deleted = (
            _delete_range(day_start, session_start)
            + _delete_range(session_end, day_end)
        )

        # Defensive verification.  If a deployment/PostgREST combination did
        # not report/delete rows from the broad range operation, identify the
        # remaining out-of-session rows and delete them by symbol + timestamp
        # range.  This also makes the operation deterministic across older
        # Supabase client versions.
        remaining_symbols = set()
        for start_utc, end_utc in ((day_start, session_start), (session_end, day_end)):
            query = self.client.table("market_candles_1m").select("symbol,candle_start")
            query = _apply_scope(query)
            response = (
                query.gte("candle_start", start_utc.isoformat())
                     .lt("candle_start", end_utc.isoformat())
        ...
        if remaining_symbols:
            # Symbol-scoped range deletes are intentionally used as the
            # fallback: they cannot accidentally delete a valid candle from a
            # different symbol and remain index-friendly.
            for symbol in remaining_symbols:
                for start_utc, end_utc in ((day_start, session_start), (session_end, day_end)):
                    query = (
                        ...
                        .delete(count="exact")
                        .eq("symbol", symbol)
                        .gte("candle_start", start_utc.isoformat())
                        .lt("candle_start", end_utc.isoformat())
                        .execute()
                    )
                    deleted += int(query.count or 0)

            # Never claim a successful Complete Day cleanup if stale rows are
            # still present.  A hard failure is safer than silently leaving
            # bad candles in the dataset.
            still_remaining = 0
            for start_utc, end_utc in ((day_start, session_start), (session_end, day_end)):
                query = self.client.table("market_candles_1m").select("symbol,candle_start", count="exact")
                query = _apply_scope(query)
                response = (
                    query.gte("candle_start", start_utc.isoformat())
                         .lt("candle_start", end_utc.isoformat())
                ...
                still_remaining += int(response.count or 0)
            if still_remaining:
                raise RuntimeError(
                    f"Complete Day cleanup could not remove {still_remaining:,} "
                    "out-of-session row(s). Check the Supabase service-role permissions."
                )
