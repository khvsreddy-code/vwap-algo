...
class TradingEngine:
    ...
    def _run_socket(self):
        """Own exactly one FYERS market-data socket.

        FYERS' SDK handles transport reconnects internally.  This thread only
        starts the socket once and retries if the SDK unexpectedly returns from
        connect().  It never races an internal SDK reconnect by constructing a
        replacement socket from a watchdog callback.
        """
        import time
        ...
        while self.running and not self.market_data_blocked:
            ...
            try:
                ...
                # Normal SDK operation keeps the socket alive through
                # keep_running(). If connect unexpectedly returns, retry after a
                # short backoff rather than closing the object a second time.
                if self.running and not self.market_data_blocked:
                    self.ws_connected = False
                    self.market_data_reconnecting = True
                    self._emit_error_once(
                        "FYERS data socket ended unexpectedly; retrying…",
                        key=("socket-ended",),
                        cooldown=30,
                    )
            except Exception as exc:
