...
class TradingEngine:
    def __init__(self, client, signal_symbol, resolution, confirmation_points, confirmation_bars,
        ...
        # Canonical live chart aggregators. Every chart timeframe is built from
        # the same FYERS tick stream; the UI never invents a candle by simply
        # moving the latest LTP onto a stale REST row.
        self._chart_current = {}
        self._chart_completed = {}
        self._chart_max_candles = 1200
    ...
    def _chart_candle_start(self, ts, resolution):
        """Canonical intraday candle bucket anchored to the algo session start."""
        ts = pd.Timestamp(ts)
        if ts.tzinfo is None:
        ...
        total = ts.hour * 60 + ts.minute
        anchor = int(self.session_start_minute)
        if total >= anchor:
            bucket = anchor + ((total - anchor) // minutes) * minutes
        else:
            bucket = (total // minutes) * minutes
    ...
    def _update_chart_aggregators(self, ltp, ts, tick_volume=0.0):
        """Build live chart candles directly from the canonical FYERS tick stream."""
        try:
            resolutions = {1, 3, 5, 10, 15, 30, 60, int(self.resolution)}
        except Exception:
            resolutions = {1, 3, 5, 10, 15, 30, 60}
        for resolution in resolutions:
            start = self._chart_candle_start(ts, resolution)
            current = self._chart_current.get(resolution)
            if current is None:
                current = {
                    "datetime": start, "open": float(ltp), "high": float(ltp),
                    "low": float(ltp), "close": float(ltp),
                    "volume": float(max(0.0, tick_volume)),
                }
                self._chart_current[resolution] = current
            elif start > current["datetime"]:
                completed = self._chart_completed.setdefault(resolution, [])
                completed.append(dict(current))
                if len(completed) > self._chart_max_candles:
                    del completed[:-self._chart_max_candles]
                current = {
                    "datetime": start, "open": float(ltp), "high": float(ltp),
                    "low": float(ltp), "close": float(ltp),
                    "volume": float(max(0.0, tick_volume)),
                ...
                current["volume"] = float(current.get("volume", 0.0)) + float(max(0.0, tick_volume))

    def display_chart_live(self, resolution):
        """Return only candles produced by the live FYERS tick stream."""
        try:
            resolution = int(resolution)
        except Exception:
            resolution = int(self.resolution)
        rows = list(self._chart_completed.get(resolution, []))
        current = self._chart_current.get(resolution)
    ...
    def _process_tick_into_candle(self, message):
        ...
        self._update_chart_aggregators(ltp, ts, tick_volume)
    ...
    def _history_bootstrap_worker(self):
        # Give the primary FYERS market-data socket a head start. Cloud option
        # chain/backfill work is secondary and can otherwise compete for REST
        # capacity during startup, causing 429s and making the terminal look
        # stuck even though live market data is healthy.
        delay = 15.0
        for _ in range(4):
    ...
    def start(self):
        ...
        self._current_candle = None
        self._last_cum_volume = None
        # A stopped/restarted engine must not carry a previous session's live
        # chart buckets into the new socket. Historical FYERS data will provide
        # the bootstrap window again, while the next websocket tick seeds the
        # current live candle.
        self._chart_current.clear()
        self._chart_completed.clear()
        self.last_tick = {}
        self.last_execution_tick = {}
        self.ws_connected = False
