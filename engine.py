    ...
    def _on_tick(self, message):
        ...
        if symbol == self.signal_symbol or not symbol:
            ...
            if current_candle is not None and not self.strategy.trade_active:
                # IMPORTANT: evaluate the raw tick against the pending setup
                # immediately. The strategy already enforces "next candle only",
                # so the crossing candle can never confirm itself. This closes a
                # real gap in the previous implementation: process_live_tick()
                # existed in the strategy but the engine never called it.
                #
                # Using the raw LTP also means a confirmation is not dependent on
                # a subsequent OHLC rebuild/Streamlit rerun. The exact tick that
                # reaches +N/-N points becomes the trigger event.
                signal = self.strategy.process_live_tick(ltp, timestamp=now)

                # Keep the candle-based path as a fallback for a broker feed that
                # updates OHLC/high-low but does not expose a usable direct tick.
                if signal is None:
                    live_df = pd.concat(
