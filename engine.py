...
class TradingEngine:
    ...
    def _repair_live_gap_worker(self):
        ...
        try:
            ...
            with self._history_lock:
                merged = pd.concat([self.history_df, repaired], ignore_index=True)
                merged["datetime"] = pd.to_datetime(merged["datetime"], errors="coerce")
                if getattr(merged["datetime"].dt, "tz", None) is None:
                    merged["datetime"] = merged["datetime"].dt.tz_localize(IST)
                else:
                    merged["datetime"] = merged["datetime"].dt.tz_convert(IST)
                merged = (
                ...
                self.history_df = VwapConfirmationEngine.prepare(merged)
                v = self.history_df.iloc[-1].get("vwap")
                if pd.notna(v):
                    self.current_vwap = float(v)
            ...
            self.events.put((
                "status",
                f"LIVE_GAP_REPAIRED:{len(repaired)} candle(s) restored from FYERS history",
            ))
            self.events.put(("candle", self.history_df.iloc[-1].to_dict()))
        except Exception as exc:
            self._emit_error_once(
