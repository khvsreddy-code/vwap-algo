...
# Market-data session ends at 15:30 IST.  Keep this separate from any
# strategy-specific "last entry" setting; the feed must not reconnect/stale
# alarm after the NSE cash session has ended.
DEFAULT_MARKET_SESSION_END_MINUTE = 15 * 60 + 30
...
def _market_data_session_active(timestamp=None):
    """Return True only during the NSE cash market-data session (09:15-15:30 IST)."""
    ts = timestamp if isinstance(timestamp, datetime) else datetime.now(IST)
    if ts.tzinfo is None:
        ts = ts.replace(tzinfo=IST)
    else:
        ts = ts.astimezone(IST)
    minute = ts.hour * 60 + ts.minute
    return DEFAULT_ENTRY_START_MINUTE <= minute < DEFAULT_MARKET_SESSION_END_MINUTE
...
class TradingEngine:
    def __init__(self, client, signal_symbol, resolution, confirmation_points, confirmation_bars,
        ...
        # Signals can be seen first on the live websocket and then again when
        # the asynchronous REST gap-repair worker replays the same completed
        # candle. Keep a small in-memory key set so replay can repair missed
        # entries without creating duplicate orders for an already-triggered
        # candle.
        self._handled_signal_keys = set()
        self._handled_signal_key_order = []
        ...
        # Live-data health is separate from the strategy/UI.  If the FYERS
        # websocket stalls, a lightweight REST quote fallback keeps the live
        # chart and strategy moving until the SDK reconnects.  It is only used
        # while the websocket is stale, so healthy websocket traffic remains
        # the primary low-latency path.
        self.data_source = "WEBSOCKET"
        self._quote_fallback_stop = threading.Event()
        self._quote_fallback_thread = None
        self._quote_fallback_active = False
        self._last_quote_fallback_at = 0.0
        self._socket_lock = threading.RLock()
        self._watchdog_stop = threading.Event()
        self._watchdog_thread = None
        self._socket_restart = threading.Event()  # compatibility; watchdog no longer closes sockets
        self._socket_generation = 0
        self._history_lock = threading.RLock()
        # Strategy state is shared by the websocket thread and the asynchronous
        # history gap-repair worker. Keep all state-machine transitions atomic
        # so a repaired candle cannot race a live confirmation tick.
        self._strategy_lock = threading.RLock()
        self._gap_repair_thread = None
        self._gap_repair_stop = threading.Event()
        ...
        self.cloud_setup_thread = None
        self.cloud_setup_stop = threading.Event()
    ...
    def load_history(self, days=31):
        ...
        if not self.history_df.empty:
            ...
            # History bootstrap can finish after the websocket has already
            # advanced several candles. In that case there was no socket
            # reconnect to trigger _schedule_live_gap_repair(), so the REST
            # bootstrap used to leave a silent hole: the chart could show a
            # HIST marker for the missed setup, but the live strategy never
            # replayed that candle and therefore never queued the entry.
            #
            # Start the same chronological repair used after reconnects as soon
            # as history becomes available. It is asynchronous and therefore
            # cannot delay the websocket or Streamlit render path.
            if self.running:
                self._schedule_live_gap_repair()
    ...
    def _process_tick_into_candle(self, message):
        ...
        if self._current_candle is None:
            # If REST history already contains this in-progress candle, seed
            # from its OHLC instead of starting at the first websocket tick
            # received after connect/reconnect. This preserves the earlier
            # portion of the candle and prevents a visually displaced bar.
            seeded = None
            try:
                hist = self.history_df
                if hist is not None and not hist.empty and "datetime" in hist.columns:
                    h = hist.copy()
                    dts = pd.to_datetime(h["datetime"])
                    ...
                    matches = dts == start
                    if matches.any():
                        row = h.loc[matches].iloc[-1]
                        seeded = {
                            "datetime": start,
                            "open": float(row["open"]),
                            "high": max(float(row["high"]), ltp),
                            "low": min(float(row["low"]), ltp),
                            "close": ltp,
                            "volume": float(row.get("volume", 0) or 0) + tick_volume,
                        }
            except Exception:
                seeded = None
            self._current_candle = seeded or {
                "datetime": start, "open": ltp, "high": ltp, "low": ltp,
                "close": ltp, "volume": tick_volume,
            }
            self._current_candle["_algo_session_first"] = (
                start.hour * 60 + start.minute
            ) == self.session_start_minute
    ...
    def _handle_strategy_signal(self, signal, source="LIVE", trigger_ltp=None):
        """Persist and queue one strategy trigger from any data source.

        Both live websocket candles and REST gap-repair candles use this exact
        path. That prevents a repaired VWAP cross from being visible on the
        chart but silently skipped by the execution engine.
        """
        if not signal:
            return
        ...
        try:
            ...
            if signal_ts.tzinfo is None:
                signal_ts = signal_ts.tz_localize(IST)
            else:
                signal_ts = signal_ts.tz_convert(IST)
            signal_key = (
                int(self._candle_start(signal_ts).timestamp()),
                str(signal.get("side", "")),
                str(signal.get("cross_type") or ""),
                round(float(signal.get("cross_price", 0.0)), 4),
                round(float(signal.get("confirmation_level", signal.get("entry", 0.0))), 4),
            )
        except Exception:
            signal_key = (
                str(signal_time),
                str(signal.get("side", "")),
                str(signal.get("cross_type") or ""),
                str(signal.get("cross_price", "")),
                str(signal.get("confirmation_level", signal.get("entry", ""))),
            )

        # Live ticks and REST gap replay can legitimately discover the same
        # confirmation twice. The first path wins; the second path must not
        # place a duplicate option order.
        if signal_key in self._handled_signal_keys:
            self._log(
                f"ENTRY DEDUPED • {source} replay matched an already-handled "
                f"signal at {signal_time}",
                "info",
            )
            return
        self._handled_signal_keys.add(signal_key)
        self._handled_signal_key_order.append(signal_key)
        if len(self._handled_signal_key_order) > 500:
            stale = self._handled_signal_key_order.pop(0)
            self._handled_signal_keys.discard(stale)

        if not _within_entry_window(
            signal_time, self.session_start_minute, self.session_end_minute
        ):
            ...
            try:
                ...
                pass
            self._log(
                f"ENTRY BLOCKED • outside configured algo entry window (IST) • source={source}",
                "info",
            )
            return
        ...
        side_name = "CE" if signal["side"] == "BUY" else "PE"
        price_text = (
            f"{float(trigger_ltp):.2f}" if trigger_ltp is not None
            else f"{float(signal.get('entry', 0)):.2f}"
        )
        self._log(
            f"ENTRY TRIGGERED [{source}] • BUY {side_name} • "
            f"price {price_text} • VWAP cross close {float(signal.get('cross_price', 0)):.2f} • "
            f"confirmation {float(signal.get('confirmation_level', 0)):.2f} • "
            f"bar {int(signal.get('bars_since_cross', 0))}",
            "entry",
        )
        trigger_event = self._record_trigger_event(signal)
        self.execute_signal(signal, trigger_event=trigger_event)

        # `trade_active` is only a duplicate guard for the just-fired setup.
        # Reset it after queueing so the next independent VWAP cycle can arm.
        self.strategy.reset_trade()
    ...
    def _accept_closed_live_candle(self, candle):
        ...
        with self._history_lock:
            self.history_df = pd.concat(
                [self.history_df, pd.DataFrame([candle])], ignore_index=True
            )
            self.history_df = (
                self.history_df
        ...
        with self._strategy_lock:
            ...
            if signal:
                self._handle_strategy_signal(signal, source="LIVE_CANDLE")
    ...
    def _fetch_cloud_chain(self):
        """Fetch the current-expiry option chain without request bursts.

        Reuse the first response when it already contains optionsChain. If FYERS
        rate-limits the request (429), retry with exponential backoff. This helper
        is only used by background cloud workers, never by the market-data tick
        callback.
        """
        import time
        ...
        for attempt in range(5):
            try:
                ...
                last_exc = exc
                text = str(exc).lower()
                if "429" not in text and "request limit" not in text and "rate limit" not in text:
                    raise
                if attempt >= 4:
                    break
                time.sleep(delay)
        ...
        raise RuntimeError(
            f"FYERS option-chain rate limit persisted after retries: {last_exc}"
        ) from last_exc
    ...
    def _configure_cloud_recorder(self):
        ...
        try:
            ...
            # Cloud recording is supplemental to the live market-data socket.
            # A temporary Supabase/FYERS 429 must never prevent NIFTY ticks,
            # candles, VWAP, or the strategy engine from starting.
            if required:
                self.events.put(("status", "CLOUD_DATA_DEGRADED:live_feed_unblocked:retrying"))
    ...
    def _cloud_backfill_today(self):
        ...
        try:
            ...
            try:
                ...
                # Only regular NSE-session candles belong in the cloud dataset.
                # 15:30 IST is exclusive; 15:29 is the final valid 1-minute bar.
    ...
    def _cloud_setup_worker(self):
        """Initialize cloud recording outside the live websocket startup path."""
        import time
        delay = 5.0
        while self.running and not self.cloud_setup_stop.is_set() and self.cloud_recorder is None:
            try:
                ...
                self._emit_error_once(
                    f"Cloud setup retryable; live feed remains active: {exc}",
                    key=("cloud-setup", str(exc)),
                    cooldown=60,
                )
            if self.cloud_setup_stop.wait(delay):
                return
            delay = min(120.0, delay * 2.0)

    def _history_bootstrap_worker(self):
        delay = 2.0
        for _ in range(4):
            if not self.running or self._history_bootstrap_stop.is_set():
                return
            try:
                df = self.load_history()
                if df is not None and not df.empty:
                    self.events.put(("status", f"HISTORY_READY:{len(df)}"))
                    return
            except Exception as exc:
                self._emit_error_once(
                    f"Initial history unavailable; live feed remains active: {exc}",
                    key=("history_bootstrap", str(exc)),
                    cooldown=60,
                )
            if self._history_bootstrap_stop.wait(delay):
                return
    ...
    def start(self):
        ...
        self.data_source = "WEBSOCKET"
        self._quote_fallback_active = False
        self._quote_fallback_stop.clear()
        self.running = True
        self._start_execution_worker()
        if self.history_df.empty:
            self._history_bootstrap_stop = getattr(
                self, "_history_bootstrap_stop", threading.Event()
            )
            self._history_bootstrap_stop.clear()
            self._history_bootstrap_thread = threading.Thread(
                target=self._history_bootstrap_worker,
                name="fyers-history-bootstrap",
                daemon=True,
            )
            self._history_bootstrap_thread.start()
        # NEVER block live websocket startup on Supabase or Option Chain REST.
        # Cloud setup can hit FYERS 429s and can take seconds; the NIFTY socket
        # must remain independent so the chart/strategy starts immediately.
        self.cloud_setup_stop.clear()
        self.cloud_setup_thread = threading.Thread(
            target=self._cloud_setup_worker,
            name="fyers-cloud-setup",
            daemon=True,
        )
        self.cloud_setup_thread.start()

        self._watchdog_stop.clear()
        self._gap_repair_stop.clear()
        ...
        self._quote_fallback_thread = threading.Thread(
            target=self._quote_fallback_loop,
            name="fyers-vwap-quote-fallback",
            daemon=True,
        )
        self._quote_fallback_thread.start()
        self.events.put(("status", "Engine started"))

    def stop(self):
        self.running = False
        self._watchdog_stop.set()
        self._quote_fallback_stop.set()
        self._socket_restart.set()
        self._gap_repair_stop.set()
        self.execution_stop.set()
        if hasattr(self, "_history_bootstrap_stop"):
            self._history_bootstrap_stop.set()
        history_worker = getattr(self, "_history_bootstrap_thread", None)
        if history_worker is not None and history_worker.is_alive():
            history_worker.join(timeout=0.2)
        self._history_bootstrap_thread = None
        sock = self.socket
        self.socket = None
        try:
        ...
        self.execution_thread = None
        fallback_worker = self._quote_fallback_thread
        if fallback_worker is not None and fallback_worker.is_alive():
            fallback_worker.join(timeout=0.5)
        self._quote_fallback_thread = None
        self._quote_fallback_active = False
        self.data_source = "STOPPED"
        self.cloud_setup_stop.set()
        if self.cloud_setup_thread is not None and self.cloud_setup_thread.is_alive():
            self.cloud_setup_thread.join(timeout=0.5)
        self.cloud_setup_thread = None
    ...
    def _on_ws_connect(self, sock=None):
        # Keep the live socket reference as soon as FYERS authenticates.
        # FYERS calls this callback again after every SDK-managed reconnect.
        if sock is not None:
            with self._socket_lock:
                self.socket = sock
        ...
        # Re-subscribe the complete desired symbol set after every reconnect.
        # The callback is the single source of truth for subscriptions; do not
        # create another socket just to restore an option subscription.
        if sock is not None:
            try:
                self.client.subscribe_data_socket(sock, sorted(self.data_symbols), "SymbolUpdate")
            except Exception as exc:
                self.market_data_error = str(exc)

        self.events.put(("status", "AUTHENTICATED"))
        # A reconnect can skip an arbitrary number of websocket ticks. Repair
        # those completed candles asynchronously from FYERS History so REST
        # latency never blocks the websocket callback.
        self._schedule_live_gap_repair()
    ...
    def _quote_fallback_loop(self):
        """Keep live LTP/strategy alive when the FYERS websocket is stale.

        This is an emergency transport fallback, not a second live feed. REST
        quotes are queried only after the websocket has stopped delivering for
        several seconds and are rate-limited to one request per ~3 seconds. A
        healthy websocket therefore remains completely unaffected.
        """
        import time

        while self.running and not self._quote_fallback_stop.wait(1.0):
            if self.market_data_blocked:
                continue

            now = datetime.now(IST)
            if not _market_data_session_active(now):
                self._quote_fallback_active = False
                if self.data_source != "STOPPED":
                    self.data_source = "OFF_SESSION"
                continue

            tick_age = self.tick_age_seconds()
            # Give the websocket a generous grace period. Normal NIFTY ticks
            # should always win; the REST path is only a recovery bridge.
            if tick_age is not None and tick_age <= 5.0:
                if self._quote_fallback_active:
                    self._quote_fallback_active = False
                    self.data_source = "WEBSOCKET"
                    self.market_data_reconnecting = False
                    self.events.put(("status", "MARKET_DATA_WEBSOCKET_RECOVERED"))
                continue

            if time.monotonic() - self._last_quote_fallback_at < 3.0:
                continue

            try:
                response = self.client.quotes([self.signal_symbol])
                rows = response.get("d", []) if isinstance(response, dict) else []
                row = rows[0] if rows and isinstance(rows[0], dict) else None
                values = row.get("v", row) if isinstance(row, dict) else {}
                ...
                if ltp is None:
                    continue
                ltp = float(ltp)
                ts = values.get("tt") or values.get("timestamp") or values.get("last_traded_time")
                try:
                    ts = float(ts) if ts is not None else now.timestamp()
                except (TypeError, ValueError):
                    ts = now.timestamp()

                self._last_quote_fallback_at = time.monotonic()
                was_active = self._quote_fallback_active
                self._quote_fallback_active = True
                ...
                if not was_active:
                    self.events.put((
                        "status",
                        "MARKET_DATA_REST_FALLBACK",
                    ))
                    # Restore any completed candles missed while the socket was
                    # stalled. The repair is asynchronous and never blocks this
                    # lightweight quote path.
                    self._schedule_live_gap_repair()
                ...
                self._on_tick({
                    ...
                    "ltp": ltp,
                    "last_traded_time": ts,
                    "vol_traded_today": values.get("volume") or values.get("vol_traded_today"),
                })
                # _on_tick marks a transport tick as connected by design. Put
                # the actual transport health back to REST_FALLBACK afterwards
                # so the UI does not falsely claim the websocket is healthy.
                self.ws_connected = False
                self.market_data_reconnecting = True
                self.data_source = "REST_FALLBACK"
            except Exception as exc:
                self._last_quote_fallback_at = time.monotonic()
                self._emit_error_once(
                    f"REST quote fallback unavailable; waiting for FYERS websocket: {exc}",
                    key=("quote-fallback", str(exc)),
                    cooldown=30,
                )


    def _socket_watchdog(self):
        """Detect a market-data socket that is no longer delivering ticks without tearing down the FYERS SDK from another thread.

        The application owns socket lifecycle/reconnects. The watchdog only
        signals the socket-owner thread; it never calls the SDK shutdown method
        itself, because doing so can race the SDK's internal message-queue worker.
        """
        import time
        ...
        while self.running and not self._watchdog_stop.wait(2.0):
            ...
            # IMPORTANT: do not require ws_connected here. FYERS can report the
            # transport as closed while its SDK is stuck inside its own automatic
            # reconnect loop. In that state the old watchdog used to skip this
            # block forever, leaving the UI with a frozen last tick.
            tick_age = self.tick_age_seconds()
            if tick_age is None or tick_age <= 15:
                continue

            now = datetime.now(IST)
            # NIFTY cash/index feed is expected only during the regular
            # 09:15-15:30 IST session.  After 15:30, a broker socket closing is
            # normal and must not be reported as a stale-feed failure.
            if not _market_data_session_active(now):
                self.market_data_reconnecting = False
                continue

            # Do not call close_connection() or create a second socket here.
            # The FYERS SDK owns reconnects for the one live socket.  Closing it
            # from the watchdog was the race that produced the SDK's
            # message_thread_stop_event=None traceback.  Instead, mark the feed
            # stale and let the SDK reconnect while a REST gap-repair worker
            # restores any candles missed during the outage.
            self.ws_connected = False
            self.market_data_reconnecting = True
            self._emit_error_once(
                f"Market data stale for {tick_age:.0f}s; waiting for SDK reconnect + history repair…",
                key=("stale",),
                cooldown=30,
            )
            self.events.put(("status", "MARKET_DATA_STALE"))
            self._schedule_live_gap_repair()

    def _run_socket(self):
        """Own one FYERS market-data socket without fighting SDK auto-reconnect.

        FYERS' SDK already performs transport reconnects when ``reconnect=True``.
        The outer loop is only a last-resort restart if ``connect()`` actually
        returns.  It uses exponential backoff and is completely disabled outside
        the 09:15-15:30 IST market-data session.
        """
        import time
        ...
        while self.running and not self.market_data_blocked:
            # There is no reason to repeatedly create/tear down a market-data
            # socket after the NSE cash session has ended.  This was the source
            # of the visible "socket ended unexpectedly; retrying..." storm at
            # 15:44 and similar post-market times.
            now = datetime.now(IST)
            if not _market_data_session_active(now):
                self.ws_connected = False
                self.market_data_reconnecting = False
                self.last_ws_event = now
                if self._watchdog_stop.wait(15.0):
                    break
                continue
            ...
            try:
                ...
                # In normal operation FYERS' SDK owns reconnects internally and
                # this call does not return.  If it does return, do not hammer
                # the broker with a new socket every 2 seconds.
                if self.running and not self.market_data_blocked:
                    self.ws_connected = False
                    self.market_data_reconnecting = True
                    self._schedule_live_gap_repair()
                    self._emit_error_once(
                        "FYERS data socket ended; waiting before a clean reconnect…",
                        key=("socket-ended",),
                        cooldown=60,
                    )
                    retry_delay = min(max_retry_delay, max(5.0, retry_delay * 1.8))
                ...
                lower = text.lower()
                if any(x in lower for x in (
                    "invalid access token", "authentication failed", "unauthorized",
                    "invalid token", "-16", "subscription failed", "11011",
                )):
                    self.market_data_blocked = True
                    self.market_data_reconnecting = False
                    self._emit_error_once(
                    ...
                    self._schedule_live_gap_repair()
                    self._emit_error_once(
                        f"Market data socket failed; retrying with backoff: {exc}",
                        key=("socket", text),
                        cooldown=60,
                    )
                    retry_delay = min(max_retry_delay, max(5.0, retry_delay * 1.8))
                ...
                if self.running and not self.market_data_blocked:
                    # Do not restart immediately.  The SDK may have just
                    # completed an internal reconnect/cleanup cycle.
                    if self._watchdog_stop.wait(retry_delay):
                        break
    ...
    def _schedule_live_gap_repair(self):
        """Start one asynchronous REST repair for candles missed by a reconnect."""
        if not self.running or self.market_data_blocked:
            return
        with self._history_lock:
            worker = self._gap_repair_thread
            if worker is not None and worker.is_alive():
                return
            ...
            worker = threading.Thread(
                target=self._repair_live_gap_worker,
                name="fyers-live-gap-repair",
                daemon=True,
            )
            self._gap_repair_thread = worker
            worker.start()

    def _repair_live_gap_worker(self):
        """Backfill completed candles missed during a websocket outage.

        The worker is deliberately outside the websocket callback.  It uses the
        existing FYERS History rate limiter and merges only candles strictly
        between the last known candle and the current live candle.  The current
        in-progress candle is left to the websocket so its OHLC remains live.
        """
        try:
            if self._gap_repair_stop.is_set() or not self.running:
                return

            with self._history_lock:
                base = self.history_df.copy() if self.history_df is not None else pd.DataFrame()
                current = dict(self._current_candle) if isinstance(self._current_candle, dict) else None
            if base.empty or "datetime" not in base.columns:
                return

            dts = pd.to_datetime(base["datetime"], errors="coerce")
            if getattr(dts.dt, "tz", None) is None:
                dts = dts.dt.tz_localize(IST)
            else:
                dts = dts.dt.tz_convert(IST)
            base = base.assign(datetime=dts).dropna(subset=["datetime"]).sort_values("datetime")
            last_known = base["datetime"].iloc[-1]
            ...
            target_start = (
                pd.Timestamp(current.get("datetime"))
                if current and current.get("datetime") is not None
                else pd.Timestamp(self._candle_start(now))
            )
            if target_start.tzinfo is None:
                target_start = target_start.tz_localize(IST)
            else:
                target_start = target_start.tz_convert(IST)

            step = pd.Timedelta(minutes=self._timeframe_minutes())
            if target_start <= last_known + step:
                return

            # Fetch the selected resolution for the current IST trading date.
            repaired = self.client.history_for_date(
                self.signal_symbol,
                self.resolution,
                now.date(),
            ...
            rdts = pd.to_datetime(repaired["datetime"], errors="coerce")
            if getattr(rdts.dt, "tz", None) is None:
                rdts = rdts.dt.tz_localize(IST)
            else:
                rdts = rdts.dt.tz_convert(IST)
            repaired["datetime"] = rdts
            repaired = repaired.dropna(subset=["datetime"])
            repaired = repaired[
                (repaired["datetime"] > last_known)
                & (repaired["datetime"] < target_start)
                & (repaired["datetime"] >= session_start)
                & (repaired["datetime"] < session_end)
            ].copy()
            if repaired.empty or self._gap_repair_stop.is_set() or not self.running:
                return

            # Normalize the OHLCV columns to the live/history schema.
            for col in ("open", "high", "low", "close", "volume"):
                if col in repaired.columns:
                    repaired[col] = pd.to_numeric(repaired[col], errors="coerce")
            repaired = repaired.dropna(subset=["open", "high", "low", "close"])
            if repaired.empty:
                return

            # IMPORTANT: repairing the chart/history alone is not enough.
            # Replay every missing completed candle through the same strategy
            # state machine, in chronological order. This reconstructs a VWAP
            # cross that happened during the websocket outage and can therefore
            # trigger the normal +15/-15 confirmation if the following repaired
            # candle reached the level.
            repaired = repaired.sort_values("datetime").reset_index(drop=True)
            repaired_count = 0

            for _, repaired_candle in repaired.iterrows():
                if self._gap_repair_stop.is_set() or not self.running:
                    break

                # _accept_closed_live_candle appends/prepares the candle and
                # advances the strategy atomically. It also uses the same
                # execution path when a repaired candle confirms an armed setup.
                with self._strategy_lock:
                    before_count = len(self.history_df)
                    self._accept_closed_live_candle(repaired_candle.copy())
                    after_count = len(self.history_df)
                    repaired_count += max(0, after_count - before_count)
            ...
            with self._history_lock:
                if self.history_df is not None and not self.history_df.empty:
                    ...
                    if pd.notna(v):
                        self.current_vwap = float(v)
                    latest_row = self.history_df.iloc[-1].to_dict()
                else:
                    latest_row = None

            self.events.put((
                "status",
                f"LIVE_GAP_REPAIRED:{len(repaired)} candle(s) restored + replayed through strategy",
            ))
            if latest_row is not None:
                self.events.put(("candle", latest_row))
        except Exception as exc:
            self._emit_error_once(
                f"Live candle gap repair failed: {exc}",
                key=("gap-repair", str(exc)),
                cooldown=60,
            )
        finally:
            with self._history_lock:
    ...
    def _on_tick(self, message):
        ...
        if symbol == self.signal_symbol:
            ...
            if current_candle is not None:
                with self._strategy_lock:
                    if not self.strategy.trade_active:
                        # Latency-critical path: if a setup is armed, evaluate
                        # every raw LTP immediately. This preserves instant
                        # +15/-15 confirmation.
                        signal = self.strategy.process_live_tick(ltp, timestamp=now)

                        # Cross detection is a closed-candle operation. Rebuild
                        # the prepared history/VWAP row only after rollover.
            ...
            if signal:
                self._handle_strategy_signal(signal, source="LIVE_TICK", trigger_ltp=ltp)
