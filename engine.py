...
class TradingEngine:
    ...
    def _run_socket(self):
        # Keep the market-data connection alive. FYERS calls keep_running()
        # inside the on_connect callback, so start_data_socket blocks until
        # the socket closes. Reconnect automatically while the engine is on.
        import time
        while self.running:
            try:
        ...
        try:
            ...
            if self.live_trading:
                ...
                if isinstance(result, dict) and result.get("s") not in ("ok",):
                    raise RuntimeError(f"FYERS order rejected: {result}")
                self._log(
            ...
            self._log(f"ENTRY FAILED • {exc}", "error")
