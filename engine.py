...
class TradingEngine:
    ...
    def _socket_watchdog(self):
        ...
        while self.running and not self._watchdog_stop.wait(2.0):
            ...
            try:
                # This is deliberately a hard lifecycle break. It wakes the
                # SDK's keep_running() so _run_socket can create a fresh
                # FyersDataSocket instead of waiting indefinitely for FYERS'
                # internal reconnect loop to recover.
                if hasattr(sock, "close_connection"):
                    sock.close_connection()
                elif hasattr(sock, "close"):
                    sock.close()
