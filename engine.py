...
class TradingEngine:
    ...
    def _socket_watchdog(self):
        ...
        FYERS has SDK-level reconnects.  The watchdog only detects stale data and
        never calls the SDK shutdown method from this thread, because that can
        race the SDK's internal message-queue worker.  Socket teardown/recreation
        remains owned by the socket thread lifecycle.
        """
        import time
        ...
        while self.running and not self._watchdog_stop.wait(2.0):
            ...
            # IMPORTANT: never call FYERS close_connection() from this
            # watchdog.  The SDK owns an internal __process_message_queue
            # thread; close_connection() can clear message_thread_stop_event
            # before that worker exits, causing:
            #   AttributeError: 'NoneType' object has no attribute 'is_set'
            # The FYERS SDK's own reconnect machinery must own socket teardown.
            # We only mark the feed stale here.  _run_socket will only create a
            # replacement after start_data_socket() actually returns, so there
            # can never be two live SDK sockets competing for the same feed.
            self.events.put(("status", "MARKET_DATA_STALE_WAITING_FOR_SDK_RECONNECT"))
