...
class TradingEngine:
    def __init__(self, client, signal_symbol, resolution, confirmation_points, confirmation_bars,
        ...
        # current ATM ladder: 20 PE strikes at/below spot and 20 CE strikes
        # at/above spot. A roll is triggered only when spot crosses the current
    ...
    def _cloud_universe_needs_roll(self, spot):
        ...
        if lo is None or hi is None:
            return True
        # The cloud universe is retained permanently. We only refresh discovery
        # when spot approaches/leaves the current FYERS strike window, so newly
        # relevant contracts can be ADDED without unsubscribing historical ones.
        width = max(1.0, float(hi) - float(lo))
        margin = max(50.0, width * 0.05)
    ...
    def _request_cloud_universe_roll(self, spot):
        ...
        with self._cloud_universe_lock:
            ...
            self._cloud_roll_requested_at = datetime.now(IST)
        ...
        while self.running and self.cloud_recorder is not None:
            ...
            try:
                ...
                if added:
                    ...
                    if self.running and not self.cloud_backfill_stop.is_set():
                        threading.Thread(
                            target=self._cloud_backfill_symbols,
                            args=(sorted(added),),
                            name="fyers-cloud-chain-backfill",
                            daemon=True,
                        ).start()
                else:
                    self._apply_cloud_oi_snapshots(selected_items, discovered)
    ...
    def _cloud_backfill_symbols(self, symbols):
        """Backfill today's 1m candles for contracts newly entering the universe."""
        today = datetime.now(IST).date()
        for symbol in list(symbols or []):
            if self.cloud_backfill_stop.is_set() or not self.running:
                return
            try:
                df = self.client.history(symbol, "1", days=3, oi_flag=True)
                ...
                df = df.copy()
                df["datetime"] = dts
                prior = df[df["datetime"].dt.date < today]["oi"].dropna() if "oi" in df.columns else pd.Series(dtype=float)
                session_base_oi = int(float(prior.iloc[-1])) if not prior.empty else None
                ...
                self._emit_error_once(
                    f"Cloud roll backfill failed for {symbol}: {exc}",
                    key=("cloud-roll-backfill", symbol, str(exc)),
                    cooldown=120,
                )
