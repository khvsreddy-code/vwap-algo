...
class TradingEngine:
    def __init__(self, client, signal_symbol, resolution, confirmation_points, confirmation_bars,
        ...
        # chart and strategy so all 40 option contracts can be recorded even
    ...
    def _configure_cloud_recorder(self):
        """Connect Supabase and subscribe to NIFTY + 20 CE + 20 PE.

        If Supabase credentials are present, startup performs a real health check
        and a real instrument upsert before the live socket starts. When the
        recorder is configured as required, a database failure stops startup
        instead of pretending that data is being saved.
        """
        ...
        try:
            ...
            first = self.client.option_chain(self.option_cfg["underlying"], strikecount=max(25, int(self.option_cfg["strikecount"])), greeks=False)
            ...
            chain_resp = self.client.option_chain(
                ...
                strikecount=max(25, int(self.option_cfg["strikecount"])),
            ...
            # Prefer the underlying row returned by the option-chain endpoint;
            # otherwise use the current NIFTY quote. This chooses the 20 nearest
            # active strikes on each side without relying on hard-coded strikes.
            spot = None
            for item in chain:
                if str(item.get("option_type") or "") == "" and item.get("ltp") is not None:
                    try:
                        spot = float(item.get("ltp"))
                        break
            ...
            if spot is None:
                try:
                    ...
                    if d and isinstance(d[0], dict):
                        v = d[0].get("v", d[0])
                        spot = float(v.get("lp")) if v.get("lp") is not None else None
                except Exception:
                    spot = None
            if spot is None:
            ...
            for item in chain:
                ...
                if typ not in candidates or not symbol:
                    continue
                try:
                    strike_f = float(strike)
                except (TypeError, ValueError):
                    continue
                candidates[typ].append((abs(strike_f - spot), strike_f, symbol, item))

            selected = []
            for typ in ("CE", "PE"):
                candidates[typ].sort(key=lambda x: (x[0], x[1]))
                selected.extend([x for x in candidates[typ][:20]])
            if len([x for x in selected if x[2]]) < 40:
                raise RuntimeError(
                    f"FYERS returned fewer than 40 usable option contracts ({len(selected)}). "
                    "Increase the option-chain strike count."
                )

            meta = {
                self.signal_symbol: {
                    "symbol": self.signal_symbol,
                    "underlying": self.option_cfg["underlying"],
                    "expiry": None,
                    "strike": None,
                    "option_type": None,
                }
            ...
            for _, strike, symbol, item in selected:
                meta[symbol] = {
                    "symbol": symbol,
                    "underlying": self.option_cfg["underlying"],
                    "expiry": str(expiry.get("expiry")),
                    "strike": strike,
            ...
            recorder.start()  # real Supabase write of the instrument set
            ...
            # Seed OI from the same option-chain response immediately, then keep
            # it refreshed once per minute in a separate thread. FYERS' SymbolUpdate
            # stream is the price/volume feed; option-chain supplies OI/OI change.
            selected_set = selected_symbols
            for item in chain:
                symbol = str(item.get("symbol") or "").strip()
    ...
    def _cloud_backfill_today(self):
        """Backfill today's 1-minute candles after the cloud recorder is online.

        This makes the recorder resilient to starting the app late in the
        session. FYERS REST history is the recovery path; the WebSocket remains
        the real-time path. Both write to the same unique (symbol, minute) key.
        """
        try:
            today = datetime.now(IST).date()
            symbols = [self.signal_symbol] + sorted(self.cloud_data_symbols)
            for symbol in symbols:
                if self.cloud_backfill_stop.is_set() or not self.running:
                    return
                is_option = symbol in self.cloud_data_symbols
                try:
                    df = self.client.history(symbol, "1", days=3, oi_flag=is_option)
                    ...
                    df = df.copy()
                    df["datetime"] = dts
                    session_base_oi = None
                    if is_option and "oi" in df.columns:
                        prior = df[df["datetime"].dt.date < today]["oi"].dropna()
                        if not prior.empty:
                            session_base_oi = int(float(prior.iloc[-1]))
                    day = df[df["datetime"].dt.date == today].copy()
                    if day.empty:
                        continue
                    rows = []
                    meta = self.cloud_recorder.instrument_meta.get(symbol, {})
                    for _, r in day.iterrows():
                        oi = _as_int(r.get("oi")) if is_option else None
                        # FYERS option-chain `oich` is the session change in OI
                        # versus the previous session, not a one-minute delta.
                        # For historical candles we reconstruct the same concept
                        # from the previous trading session's final OI.
                        oi_change = (oi - session_base_oi) if (oi is not None and session_base_oi is not None) else None
                        rows.append({
                            **meta,
                    ...
                    self._emit_error_once(
                        f"Cloud historical backfill failed for {symbol}: {exc}",
                        key=("cloud-backfill", symbol, str(exc)),
                        cooldown=120,
                    )
        finally:
    ...
    def start(self):
        ...
        # socket subscribes to NIFTY + all 40 tracked option contracts from its
