...
class TradingEngine:
    ...
    def _run_socket(self):
        # FYERS normally reconnects internally, but the SDK can also return from
        # connect()/keep_running() after a transport failure.  Keep the engine
        # alive and re-create the market-data socket in that case so a temporary
        # "remote host was lost" event cannot freeze both live chart pages.
        import time
        ...
        while self.running and not self.market_data_blocked:
            try:
                ...
                # A normal FYERS socket stays inside the SDK's keep_running().
                # If it returns unexpectedly, immediately enter the controlled
                # retry path below instead of leaving a dead engine thread.
                if self.running and not self.market_data_blocked:
                    self.ws_connected = False
                    self.market_data_reconnecting = True
                    self._emit_error_once(
                        "Market data socket ended; reconnecting…",
                        key=("socket-ended",),
                        cooldown=60,
                    )
            except Exception as exc:
                ...
                sock = self.socket
                self.socket = None
                if sock is not None and self.running and not self.market_data_blocked:
