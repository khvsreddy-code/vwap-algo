...
class TradingEngine:
    def __init__(self, client, signal_symbol, resolution, confirmation_points, confirmation_bars,
        ...
        self._socket_restart = threading.Event()
    ...
    def _socket_watchdog(self):
        ...
        while self.running and not self._watchdog_stop.wait(2.0):
            ...
            sock = self.socket
            if sock is None:
                # _run_socket may already be between SDK reconnect attempts.
                # Leave it alone and let the outer owner create the next socket.
                self.market_data_reconnecting = True
                continue
            ...
            # Ask the socket-owner thread to tear down this socket and let the
            # outer _run_socket loop create exactly one replacement. The
            # watchdog NEVER calls close_connection() itself; that was the race
            # that could leave FYERS' __process_message_queue thread with
            # message_thread_stop_event=None and produce the NoneType traceback.
            self._socket_restart.set()
            self.events.put(("status", "MARKET_DATA_STALE_RESTARTING"))

    def _run_socket(self):
        """Own one long-lived market-data socket and recreate it after hard exits.

        The application owns market-data reconnects. A fresh FYERS socket is
        created after a transport failure or watchdog-detected stale feed,
        instead of relying on the SDK's internal reconnect worker.
        """
        import time
        ...
        while self.running and not self.market_data_blocked:
            ...
            try:
                ...
                sock = self.client.start_data_socket(
                    ...
                    restart_event=self._socket_restart,
                )
