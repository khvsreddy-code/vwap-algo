...
def test_socket_close_callback_does_not_race_sdk_reconnect():
    src = _text("fyers_client.py")
    assert "lifecycle_done.set()" in src
    close_block = src.split("def _close(msg=None):", 1)[1].split("# Let FYERS own", 1)[0]
    assert "lifecycle_done.set()" not in close_block
    assert "reconnect_retry=50" in src


def test_connect_replaces_old_engine_cleanly():
    src = _text("app.py")
    assert 'previous_engine = st.session_state.get("engine")' in src
    assert "previous_engine.stop()" in src
