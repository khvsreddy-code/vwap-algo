...
When NIFTY crosses the current near-ATM boundary, the recorder refreshes the option chain off the websocket thread, computes the new 20+20 universe, subscribes to newly entering contracts, unsubscribes retired cloud contracts, refreshes OI/OI-change snapshots, and backfills today's 1-minute candles for newly entering contracts. Historical rows for retired contracts remain in Supabase.
