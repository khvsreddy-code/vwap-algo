...
v9.4.44 — live chart transport/stability + reliability hardening.
...
def make_payload(
    ...
    height=900, max_candles=90, fit_content=False, incremental=False
):
    ...
    if incremental and not fit_content and core.get("candles"):
        chart_key = f"v9_4_47::{title}::{height}"
        signature = (
            core["candles"][0]["time"],
            core["candles"][-1]["time"],
            len(core["candles"]),
        )
        previous = st.session_state.get("_v946_chart_signature", {}).get(chart_key)
        st.session_state.setdefault("_v946_chart_signature", {})[chart_key] = signature
