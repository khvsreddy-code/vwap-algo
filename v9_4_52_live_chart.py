...
<div id="vwap-live-chart" class="vwap-chart-shell">
  ...
  <div class="vwap-chart-vwap-badge">VWAP</div><div class="vwap-chart-hint">Scroll to zoom • drag to pan</div>
</div>
"""
...
export default function(component) {
  ...
  // The slot is intentionally transparent, not visibility:hidden. The portal
  // visibility check below uses computed visibility to decide whether the chart
  // should be shown. Setting visibility:hidden here therefore hid the chart
  // forever (the blank-chart regression in v9.4.85).
  ...
  // Coalesce resize/scroll/mutation observer bursts into one layout read per
  // animation frame. Streamlit tab changes can otherwise fire several observers
  // back-to-back, causing visible jank while the chart is being interacted with.
  function schedulePortalSync() {
    if (state.portalSyncRaf) return;
    state.portalSyncRaf = requestAnimationFrame(() => {
      state.portalSyncRaf = 0;
      syncPortalPosition();
    });
  }
  ...
  if (!state.portalPositionBound) {
    ...
    try {
      state.slotResizeObserver = new ResizeObserver(schedulePortalSync);
  ...
  function markerPalette(text) {
    ...
    // Exit reason always wins over CE/PE entry styling.
    // TARGET = teal; STOP LOSS = subtle red-orange.
    if (t.includes("TARGET HIT")) {
      return {
        bg: "linear-gradient(135deg, #0f766e 0%, #0d9488 100%)",
        border: "rgba(94,234,212,.78)",
        glow: "rgba(20,184,166,.34)",
        dot: "#5eead4",
        arrow: "#14b8a6"
      };
    }
    if (t.includes("STOP LOSS HIT")) {
      return {
        bg: "linear-gradient(135deg, #b45309 0%, #c2410c 100%)",
        border: "rgba(253,186,116,.78)",
  ...
  function applyPayload(p) {
    ...
    if (d.mode === "delta") {
      if (d.candle && state.candles) {
        ...
        const incomingTime = Number(d.candle.time);
        const lastTime = state.candleData.length
          ? Number(state.candleData[state.candleData.length - 1].time) : null;
        if (lastTime == null || incomingTime >= lastTime) {
          ...
          try { state.candles.update(d.candle); } catch (e) {
            try {
              state.candleData.sort((a, b) => Number(a.time) - Number(b.time));
              state.candles.setData(state.candleData);
            } catch (_) {}
          }
        } else {
          // A delayed/reconnected tick can legitimately carry an older bucket.
          // Never append it after the newest candle. Insert it in timestamp
          // order and rebuild once; this path is rare and prevents visual
          // candle displacement while the normal live path stays O(1).
          state.candleData.push(d.candle);
          state.candleData.sort((a, b) => Number(a.time) - Number(b.time));
          try { state.candles.setData(state.candleData); } catch (e) {}
        }
      }
      if (state.vwap && d.vwap) {
        state.vwapData = state.vwapData || [];
        const incomingVwapTime = Number(d.vwap.time);
        const lastVwapTime = state.vwapData.length
          ? Number(state.vwapData[state.vwapData.length - 1].time) : null;
        if (lastVwapTime == null || incomingVwapTime >= lastVwapTime) {
          if (lastVwapTime === incomingVwapTime) {
          ...
          try { state.vwap.update(d.vwap); } catch (e) {
            try {
              state.vwapData.sort((a, b) => Number(a.time) - Number(b.time));
              state.vwap.setData(state.vwapData);
            } catch (_) {}
          }
        } else {
          state.vwapData.push(d.vwap);
          state.vwapData.sort((a, b) => Number(a.time) - Number(b.time));
          try { state.vwap.setData(state.vwapData); } catch (e) {}
        }
      }
    } else {
      try {
    ...
    function snapMarkerTime(raw) {
      ...
      // candleTimes is sorted. Binary-search the nearest candle instead of
      // scanning the entire 800-bar window for every marker on every live tick.
      ...
      const right = candleTimes[lo];
      const left = lo > 0 ? candleTimes[lo - 1] : right;
      const best = Math.abs(right - t) < Math.abs(left - t) ? right : left;
      const bestDelta = Math.abs(best - t);
    ...
    // Marker text is normally unchanged across intrabar ticks. Avoid rebuilding
    // the overlay (and re-snapping timestamps) unless the marker payload itself
    // changed. This keeps the browser main thread quiet while the chart is live.
    const nextMarkerPayloadKey = JSON.stringify(d.markers || []);
    if (state.lastMarkerPayloadKey !== nextMarkerPayloadKey) {
      const markerData = (d.markers || [])
        .map(m => ({...m, time: snapMarkerTime(m.time)}))
        .filter(m => m.time != null)
        .sort((a, b) => Number(a.time) - Number(b.time));
      state.lastMarkers = markerData;
      state.lastMarkerPayloadKey = nextMarkerPayloadKey;
      state.lastMarkerPayload = (d.markers || []).slice();
      updateMarkerOverlay(state.lastMarkers);
    } else if (!state.markerOverlay) {
      updateMarkerOverlay(state.lastMarkers || []);
    }
  ...
  function drawFallback() {
    ...
    (d.markers||[]).forEach(m=>{
      ...
      const isTarget=text.includes("TARGET HIT");
      const isSL=text.includes("STOP LOSS HIT");
      const isPE=text.includes("BUY PE"), isSell=text.includes("SELL");
      const accent=isTarget?"#14b8a6":(isSL?"#f97316":(isSell?"#f59e0b":(isPE?"#8b5cf6":"#10b981")));
      const pillBg=isTarget?"rgba(13,148,136,.96)":(isSL?"rgba(194,65,12,.96)":(isSell?"rgba(146,64,14,.96)":(isPE?"rgba(91,33,182,.96)":"rgba(4,120,87,.96)")));
  ...
  function themeColors(name) {
    ...
    if (t.includes("black")) {
      return {
        ...
        vwap: "#ffd166", ltp: "#d0d0d0", hintBg: "rgba(0,0,0,.82)", hintText: "#bdbdbd"
      };
    }
    if (t.includes("light")) {
      return {
        ...
        vwap: "#d97706", ltp: "#555555", hintBg: "rgba(255,255,255,.90)", hintText: "#64748b"
      };
    }
    return {
      background: "#0b0f14", text: "#9aa4b2", grid: "#17202b", border: "#263241",
      up: "#22b39b", down: "#ef5350", wickUp: "#22b39b", wickDown: "#ef5350",
      vwap: "#ffd166", ltp: "#8b95a7", hintBg: "rgba(9,14,21,.76)", hintText: "#718096"
    };
  }
  ...
  function init() {
    ...
    // VWAP must remain visually distinct from the candles/LTP line.  Keep it
    // on the same price scale, but use a thicker high-contrast stroke and a
    // visible last-value label so it cannot disappear into the candle bodies.
  ...
  loadScript().then(() => {
    if (!state.chart && window.LightweightCharts && !state.fallbackUpgradeTried) {
      ...
      const canvas = state.fallbackCanvas;
      // Keep the already-visible fallback on screen until Lightweight Charts
      // has actually initialized. Never hide the only working renderer first.
      try {
        init();
        if (state.chart && state.ready) {
          ...
          throw new Error("chart did not initialize");
        }
      } catch (e) {
        state.fallback = true;
...
def _normalise_chart_frame(df, resolution=None):
    """Normalize chart rows before they reach Lightweight Charts.

    Timestamps are made timezone-consistent and snapped to the selected
    intraday bucket. Duplicate buckets are merged, then OHLC invariants are
    enforced so malformed rows cannot create artificial displaced wicks.
    """
    ...
    if resolution is not None:
        try:
            ...
            total = out["datetime"].dt.hour * 60 + out["datetime"].dt.minute
            anchor = 9 * 60 + 15
            bucket = anchor + ((total - anchor) // minutes) * minutes
            bucket = bucket.where(total >= anchor, (total // minutes) * minutes)
            out["datetime"] = out["datetime"].dt.normalize() + pd.to_timedelta(bucket, unit="m")
        except Exception:
            pass
    out = out.sort_values("datetime").reset_index(drop=True)
    merged = []
    for _, grp in out.groupby("datetime", sort=True):
        # REST history and the live overlay can describe the same candle.
        # The latest row is authoritative for OHLC. Taking max(high)/min(low)
        # across duplicate rows can import a stale/corrupt wick and create the
        # apparent "displaced candle" seen in the chart.
        row = grp.iloc[-1].copy()
        o = pd.to_numeric(pd.Series([row.get("open")]), errors="coerce").iloc[0]
        h = pd.to_numeric(pd.Series([row.get("high")]), errors="coerce").iloc[0]
        lo = pd.to_numeric(pd.Series([row.get("low")]), errors="coerce").iloc[0]
        c = pd.to_numeric(pd.Series([row.get("close")]), errors="coerce").iloc[0]
        row["open"] = float(o)
        row["close"] = float(c)
        row["high"] = max(float(h), float(o), float(c))
        row["low"] = min(float(lo), float(o), float(c))

        if "volume" in grp.columns:
            row["volume"] = pd.to_numeric(
                grp["volume"], errors="coerce"
            ).fillna(0).max()

        # A live overlay row may not have a VWAP yet. Preserve the latest valid
        # VWAP for this exact candle instead of replacing it with NaN.
        if "vwap" in grp.columns:
            valid_vwap = pd.to_numeric(grp["vwap"], errors="coerce").dropna()
            if not valid_vwap.empty:
                row["vwap"] = float(valid_vwap.iloc[-1])
...
def make_payload(
    ...
    if df is not None and not df.empty:
        df = _normalise_chart_frame(df, resolution=resolution)

        # Do not let a missing derived column silently produce a chart with no
        # VWAP.  Rebuild the session VWAP from OHLC4 * traded volume when the
        # caller supplied volume but forgot to run the strategy preparation.
        # This is a true volume-weighted VWAP; if volume is unavailable/zero we
        # intentionally leave VWAP absent rather than inventing one.
        if vwap and "volume" in df.columns:
            try:
                ...
                if (vol > 0).any():
                    ...
                    work = df.copy()
                    work["_vwap_vol"] = vol
                    work["_vwap_pv"] = typical * vol
                    work["_vwap_date"] = work["datetime"].dt.date
                    work["_vwap_cum_pv"] = work.groupby("_vwap_date")["_vwap_pv"].cumsum()
                    work["_vwap_cum_vol"] = work.groupby("_vwap_date")["_vwap_vol"].cumsum()
                    computed_vwap = work["_vwap_cum_pv"] / work["_vwap_cum_vol"].replace(0, pd.NA)
                    if "vwap" in work.columns:
                        existing_vwap = pd.to_numeric(work["vwap"], errors="coerce")
                        work["vwap"] = existing_vwap.combine_first(computed_vwap)
                    else:
                        work["vwap"] = computed_vwap
                    df = work.drop(columns=[
                        "_vwap_vol", "_vwap_pv", "_vwap_date",
                        "_vwap_cum_pv", "_vwap_cum_vol"
                    ])
            except Exception:
                pass
    ...
    if df is None or df.empty:
        ...
        # Fast path for the steady-state live tick: the chart only needs the
        # current candle/LTP/VWAP after the first full payload. We intentionally
        # keep the existing full-window path for first render and candle rollover.
        if incremental and not fit_content:
            chart_key = f"v9_4_52::{title}::{height}"
            prior_sig = st.session_state.get("_v952_chart_signature", {}).get(chart_key)
            try:
                last_row = df.iloc[-1]
                last_ts = int(pd.Timestamp(last_row["datetime"]).timestamp())
                row_count = min(len(df), max(1, int(max_candles or DEFAULT_CHART_CANDLES)))
                first_window = df.tail(row_count)
                first_ts = int(pd.Timestamp(first_window.iloc[0]["datetime"]).timestamp())
                signature = (first_ts, last_ts, row_count)
            except Exception:
                prior_sig = None
                signature = None

            if signature is not None and prior_sig is not None and prior_sig[0] == signature[0] and prior_sig[2] == signature[2]:
                st.session_state.setdefault("_v952_chart_signature", {})[chart_key] = signature
                try:
                    o = _json_float(last_row.get("open"))
                    h = _json_float(last_row.get("high"))
                    lo = _json_float(last_row.get("low"))
                    c = _json_float(last_row.get("close"))
                    ts = int(pd.Timestamp(last_row["datetime"]).timestamp())
                    vv = _json_float(last_row.get("vwap")) if vwap else None
                    if None not in (o, h, lo, c):
                        core = {
                            "mode": "delta",
                            "candle": {"time": ts, "open": o, "high": h, "low": lo, "close": c},
                            "vwap": {"time": ts, "value": vv} if vv is not None else None,
                            "ltp": _json_float(ltp),
                            "levels": levels or [],
                            "markers": markers or [],
                            "title": str(title),
                        ...
                        json.dumps(core, separators=(",", ":"), ensure_ascii=False, allow_nan=False)
                        return core
                except Exception:
                    pass
...
def render(
    ...
    max_candles=DEFAULT_CHART_CANDLES, fit_content=False, resolution=None
):
    payload = make_payload(
        df, vwap=vwap, ltp=ltp, levels=levels, markers=markers,
        title=title, height=height, max_candles=max_candles,
        fit_content=fit_content, incremental=(not fit_content), resolution=resolution
    )
