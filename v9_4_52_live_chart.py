...
export default function(component) {
  // IMPORTANT: size from the V2 component element itself. Its parent can be
  // Streamlit's narrow internal wrapper, which previously made Lightweight
  // Charts initialize at ~300px wide while the surrounding card was full width.
  const root = component;
  const outerRoot = component.parentElement || component;
  ...
  try {
    ...
    component.style.height = requestedHeight + "px";
    component.style.minHeight = requestedHeight + "px";
    component.style.maxHeight = requestedHeight + "px";
  ...
  function init() {
    ...
    state.chart = LightweightCharts.createChart(host, {
      width: Math.max(320, Math.round(
        ...
        outerRoot.getBoundingClientRect().width ||
        900
      )),
    ...
    const resizeChart = () => {
      ...
      const outerRect = outerRoot.getBoundingClientRect();
      // Prefer the actual V2 component width. Never fall back to a narrow
      // parent wrapper when the component itself has a larger layout box.
    ...
    state.resizeObserver.observe(component);
    state.resizeObserver.observe(host);
    if (outerRoot && outerRoot !== component) state.resizeObserver.observe(outerRoot);
    state.ready = true;
