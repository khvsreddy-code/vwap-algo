...
v9.4.61 — persistent DOM chart + true incremental updates + V2 mount fix.
...
export default function(component) {
  // Streamlit V2 passes a renderer-args object here, NOT a DOM element.
  // `parentElement` is the component's DOM root (a ShadowRoot when
  // isolate_styles=True). The old code treated `component` itself as a DOM
  // node, which caused: `BidiComponent Error: root.appendChild is not a function`.
  const { parentElement, data = {}, key = "" } = component;
  const root = parentElement;
  const mountHost = root && root.host instanceof HTMLElement ? root.host : root;
  ...
  // The Python mount uses width="stretch" and a fixed height. Streamlit sizes
  // the mount wrapper; the renderer should fill that box rather than trying to
  // size the non-DOM `component` argument.
  try {
  ...
  // Streamlit can recreate the component's ShadowRoot during a rerun. Keep
  // the actual chart DOM node in a window-level registry and move that SAME
  // node into the current parentElement. Lightweight Charts therefore keeps
  // the same chart instance instead of rebuilding it.
...
try:
    _component = st.components.v2.component(
        name="fyers_vwap_live_chart_v9_4_61",
        html=_CHART_HTML,
        css=_CHART_CSS,
        js=_CHART_JS,
        isolate_styles=True,
