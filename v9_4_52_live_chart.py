...
export default function(component) {
  ...
  let host = root.querySelector(".vwap-chart-shell");
  if (!host) {
  ...
  // If Streamlit supplied a genuinely new mount root, the old chart instance is
  // no longer attached to a usable DOM host. Dispose that browser object once and
  // rebuild it against the new host; ordinary fragment reruns keep the same host
  // and therefore keep the same chart instance.
  if (state.host !== host) {
    // Streamlit V2 can remount the component into a fresh ShadowRoot while
    // keeping the same Python key. Never carry "ready" or the old renderer
    // state across that boundary: doing so makes the new mount ignore payloads
    // because state.chart is null while state.ready is still true.
    try { state.resizeObserver && state.resizeObserver.disconnect(); } catch (e) {}
    try { state.chart && state.chart.remove(); } catch (e) {}
    try { state.markerOverlay && state.markerOverlay.remove(); } catch (e) {}
    state.host = null;
    state.root = null;
    state.chart = null;
    state.candles = null;
    ...
    state.fallbackView = { start: 0, count: 0 };
    state.fallbackDrag = null;
    state.fallbackBound = false;
    state.fallbackUpgradeTried = false;
    state.markerOverlay = null;
    state.markerOverlayBound = false;
    if (state.markerOverlayRaf) {
      try { cancelAnimationFrame(state.markerOverlayRaf); } catch (e) {}
    }
...
def render(
    ...
    # Keep the component identity stable, but give each payload a tiny sequence
    # value derived from the visible candle/LTP state. This makes live V2 renders
    # unambiguously new when only the current candle close changes.
    try:
