...
export default function(component) {
  ...
  // IMPORTANT: Streamlit V2 can give the renderer a fresh ShadowRoot on a
  // fragment rerun even when the Python component key is unchanged. The old
  // implementation treated that as a brand-new chart and destroyed/recreated
  // Lightweight Charts every second. That is the visible blink/freeze problem.
  //
  // Re-parent the existing chart DOM instead. The Lightweight Charts instance,
  // canvas, viewport, scroll position and series objects stay alive; only the
  // Streamlit mount wrapper changes. This makes the chart behave like a true
  // live terminal: incoming OHLC is applied to the existing last candle.
  if (!host && state.host) {
    host = state.host;
    try { root.appendChild(host); } catch (e) {
      host = null;
    }
  }
  ...
  // Do NOT destroy state when Streamlit supplies a new mount root. Reusing the
  // existing host is the key to eliminating the one-second chart blink.
  state.root = root;
  state.host = host;
