...
export default function(component) {
  ...
  // Streamlit V2 can leave the component wrapper with an intrinsic height that
  // is smaller than the requested component height. That makes the chart
  // canvas collapse to a thin strip while the component itself still reserves
  // the full vertical space. Own the wrapper dimensions explicitly so every
  // chart uses the full height requested by Python.
  try {
  ...
  function applyPayload(p) {
    ...
    if (!sameJson(state.lastMarkers, markerData)) {
      ...
      try {
        // v4.2.x (the bundled CDN version) owns markers directly on the series.
        if (state.candles && typeof state.candles.setMarkers === "function") {
          state.candles.setMarkers(markerData);
          markerApplied = true;
        }
      } catch (e) {}

      // Keep a v5-compatible fallback in case the CDN/runtime is upgraded.
      if (!markerApplied) {
        try {
          if (typeof LightweightCharts.createSeriesMarkers === "function") {
            if (!state.markerPlugin) {
              state.markerPlugin = LightweightCharts.createSeriesMarkers(state.candles, markerData);
            } else if (typeof state.markerPlugin.setMarkers === "function") {
      ...
      if (markerApplied) state.lastMarkers = markerData.slice();
    }
  ...
  function init() {
    ...
    state.ready = true;
    // Wait one paint so Streamlit has committed the wrapper dimensions before
    // the first Lightweight Charts layout pass.
