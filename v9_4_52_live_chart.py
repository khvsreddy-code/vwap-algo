...
def _normalise_chart_frame(df, resolution=None):
    ...
    for _, grp in out.groupby("datetime", sort=True):
        row = grp.iloc[-1].copy()
        row["open"] = grp["open"].iloc[0]
        row["high"] = max(float(grp["high"].max()), float(grp["open"].iloc[0]), float(grp["close"].iloc[-1]))
        row["low"] = min(float(grp["low"].min()), float(grp["open"].iloc[0]), float(grp["close"].iloc[-1]))
        if "volume" in grp.columns:
            row["volume"] = pd.to_numeric(grp["volume"], errors="coerce").fillna(0).max()
        merged.append(row)
