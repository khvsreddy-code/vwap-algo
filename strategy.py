...
class VwapConfirmationEngine:
    ...
    def process_closed_candle(self, row: pd.Series):
        ...
        # Detect a VWAP cross from either the previous completed candle OR
        # the current candle's own open/close.  The latter is important for
        # the first candle of a session (and for candles that open on one side
        # of VWAP, cross through it intrabar, then close on the other side).
        long_cross = (
        ...
        # A fresh cross replaces any older pending setup.  Normally the
        # confirmation happens on a later candle, but if the same candle
        # crosses VWAP and then travels the full confirmation distance, that
        # move is valid and must not be discarded.
        if long_cross:
            self._arm(1, close)
            if high >= self.confirmation_level:
                return self._make_signal("BUY", self.confirmation_level, row, 0)
            return None
        if short_cross:
            self._arm(-1, close)
            if low <= self.confirmation_level:
                return self._make_signal("SELL", self.confirmation_level, row, 0)
            return None
    ...
    def process_live_candle(self, row: pd.Series):
        ...
        Unlike ``process_closed_candle``, this method is called on every live
        tick with the partial OHLC candle.  It catches the case where the
        session's first candle (or any later candle) opens on one side of VWAP,
        crosses it intrabar, closes on the opposite side, and then reaches the
        confirmation distance before the candle closes.  It also checks the
        high/low of the current candle against an already-armed setup so a live
        move cannot be missed between ticks.
        """
        ...
        if self.cross_bar is not None or pd.isna(prev_close) or pd.isna(prev_vwap):
            return None

        # Intrabar/session-open cross detection.  This complements the normal
        # previous-close/previous-VWAP test and is what catches a first candle
        # that opens above/below VWAP and then crosses through it.
        long_cross = (
            (float(prev_close) <= float(prev_vwap) and close > float(vwap))
            or (open_price <= float(vwap) and close > float(vwap))
        )
        ...
        if long_cross:
            self._arm(1, close)
            if high >= float(self.confirmation_level):
                return self._make_signal(
                    "BUY", float(self.confirmation_level),
            ...
            self._arm(-1, close)
            if low <= float(self.confirmation_level):
                return self._make_signal(
                    "SELL", float(self.confirmation_level),
                    {"datetime": row.get("datetime")}, 0
                )
