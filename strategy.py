...
class StrategyConfig:
    ...
    # rule. They apply to softer VWAP interaction/reclaim/bounce setups.
    # A genuine close-to-close side change always arms.
    chop_lookback: int = 8
...
class VwapConfirmationEngine:
    ...
    A chop filter suppresses weak interaction/reclaim/bounce setups when VWAP
    has been crossed repeatedly in a short range. A genuine close-to-close
    side change is never suppressed: it always arms the normal confirmation
    state so valid crosses cannot be lost.
    """
    ...
    def _detect_vwap_cross(self, row, first_session_candle=False):
        ...
        try:
            ...
            if long_cross:
                # A genuine close-to-close VWAP side change is the primary
                # strategy event. Do NOT suppress it merely because the
                # preceding candles were choppy or the regime classifier says
                # RANGE. Those filters were causing valid crosses like the
                # user's circled setup to disappear before the 15-point
                # confirmation state was ever armed.
                #
                # Chop/range protection remains available for the softer
                # interaction/reclaim/bounce families below. CLOSE_CROSS is
                # intentionally deterministic: side changed -> arm it.
    ...
    def process_closed_candle(self, row: pd.Series):
        ...
        if self.cross_bar is not None and not self.trade_active:
            ...
            # A pending setup is invalid as soon as a *closed* candle closes on
            # the opposite side of VWAP. Never keep the old direction armed
            # simply because its confirmation window has not expired. This is
            # especially important for a PE/CE flip: if a bearish setup is
            # followed by a candle that closes back above VWAP, the bearish
            # setup must disappear immediately and the new bullish side must be
            # eligible from that candle's close.
            #
            # Do this BEFORE confirmation checks. Otherwise an intrabar wick
            # can confirm the stale setup on the same candle that has already
            # closed through VWAP in the opposite direction.
            if bars >= 1:
                if self.cross_direction == 1 and close < float(vwap):
                    failed_type = "FAILED_CROSS" if bars == 1 else "CLOSE_CROSS"
                    self._clear_setup()
                    self._arm(-1, close, failed_type, "MEDIUM" if bars == 1 else "HIGH")
                    self._update_relationship_state(row)
                    return None
                if self.cross_direction == -1 and close > float(vwap):
                    failed_type = "FAILED_CROSS" if bars == 1 else "CLOSE_CROSS"
