...
class StrategyConfig:
    ...
    # V10.1 quality layer. These are deliberately conservative: they reject
    # only clearly weak setups and do not impose a trade-count quota.
    use_setup_quality: bool = True
    quality_close_cross_min: int = 4
    quality_reclaim_min: int = 3
    quality_bounce_min: int = 4
    quality_interaction_min: int = 3
    quality_failed_cross_min: int = 3
    quality_body_ratio: float = 0.30
    quality_close_location: float = 0.60
    quality_min_displacement_atr: float = 0.12
    quality_strong_adx: float = 25.0
    quality_max_recent_flips: int = 2
    block_strong_countertrend: bool = True

    # V10.2 precision regime layer. The goal is to remove low-quality
    # continuation entries without changing the 15-point confirmation contract.
    use_precision_regime: bool = True
    precision_min_efficiency: float = 0.30
    precision_reversal_min_efficiency: float = 0.20
    precision_min_slope_atr: float = 0.025
    precision_max_setup_distance_atr: float = 0.90
    precision_max_flip_count: int = 2
    precision_opening_bars: int = 6
    precision_opening_max_flips: int = 2
    precision_opening_min_efficiency: float = 0.24
    precision_late_entry_hour: int = 14
    precision_late_entry_minute: int = 0
    precision_allow_strong_late_move: bool = True
...
class VwapConfirmationEngine:
    ...
    def _make_signal(self, side, entry, row=None, bars=None):
        ...
        signal = {
            ...
            "quality_score": self._setup_quality_score(row if isinstance(row, pd.Series) else pd.Series(), self.trade_direction, self.cross_type or "CLOSE_CROSS") if row is not None else None,
            "regime": self._market_regime(row) if row is not None and isinstance(row, pd.Series) else "UNKNOWN",
    ...
    def _detect_vwap_cross(self, row, first_session_candle=False):
        ...
        try:
            ...
            if long_cross:
                if not self._chop_blocks(row, 1) and self._market_regime(row) != "RANGE" and not self._quality_blocks(row, 1, "CLOSE_CROSS"):
                    return (1, "CLOSE_CROSS", "HIGH")
                return None
            if short_cross:
                if not self._chop_blocks(row, -1) and self._market_regime(row) != "RANGE" and not self._quality_blocks(row, -1, "CLOSE_CROSS"):
                    return (-1, "CLOSE_CROSS", "HIGH")
            ...
            if bullish_reclaim and not self._chop_blocks(row, 1) and self._market_regime(row) != "RANGE":
                setup_name = "VWAP_INTERACTION" if rounded else "VWAP_RECLAIM"
                if not self._quality_blocks(row, 1, setup_name):
                    return (1, setup_name, "HIGH")
            if bearish_reclaim and not self._chop_blocks(row, -1) and self._market_regime(row) != "RANGE":
                setup_name = "VWAP_INTERACTION" if rounded else "VWAP_RECLAIM"
                if not self._quality_blocks(row, -1, setup_name):
                    return (-1, setup_name, "HIGH")
            if bullish_bounce and not self._chop_blocks(row, 1) and self._market_regime(row) != "RANGE" and not self._quality_blocks(row, 1, "VWAP_BOUNCE"):
                return (1, "VWAP_BOUNCE", "MEDIUM")
            if bearish_bounce and not self._chop_blocks(row, -1) and self._market_regime(row) != "RANGE" and not self._quality_blocks(row, -1, "VWAP_BOUNCE"):
                return (-1, "VWAP_BOUNCE", "MEDIUM")
    ...
    def _setup_quality_score(self, row, direction, setup_type):
        """Score the setup using only information available on this candle.

        This is intentionally a *quality* layer, not a new confirmation rule.
        A missing warm-up indicator never causes rejection.
        """
        if not self.config.use_setup_quality:
            return None
        ...
        if any(x is None for x in (close, vwap, high, low)):
            return None

        score = 0
        spread = max(high - low, 1e-9)
        body = abs(close - (open_price if open_price is not None else close))
        body_ratio = body / spread
        close_location = (
            (close - low) / spread if direction == 1
            else (high - close) / spread
        )

        # Price should finish on the intended side of VWAP.
        if (direction == 1 and close > vwap) or (direction == -1 and close < vwap):
            score += 2

        # A VWAP slope aligned with the proposed direction is a useful
        # continuation-quality signal, but is not mandatory for reversals.
        if slope is not None:
            if (direction == 1 and slope > 0) or (direction == -1 and slope < 0):
                score += 1

        # Strong trend strength earns a point.
        if adx is not None and adx >= float(self.config.quality_strong_adx):
            score += 1

        # Meaningful displacement is better than a tiny VWAP cross.
        if atr is not None and atr > 0 and abs(close - vwap) >= atr * float(self.config.quality_min_displacement_atr):
            score += 1

        if body_ratio >= float(self.config.quality_body_ratio):
            score += 1
        if close_location >= float(self.config.quality_close_location):
            score += 1

        recent_flips = sum(
            1 for a, b in zip(self._recent_relationships[-8:-1],
                              self._recent_relationships[-7:])
            if a and b and a != b
        )
        if recent_flips <= int(self.config.quality_max_recent_flips):
            score += 1

        # Reversal families intentionally get a little more tolerance because
        # their edge is the failure/reclaim itself rather than trend alignment.
        if setup_type in ("FAILED_CROSS", "VWAP_RECLAIM", "VWAP_INTERACTION"):
            score += 1

        return int(score)

    def _precision_metrics(self, row):
        """Return normalized, causal market-quality metrics for this candle.

        These metrics are deliberately based only on the completed candle and
        precomputed history. No future bars are referenced.
        """
        atr = self._safe_float(row.get("atr"))
        close = self._safe_float(row.get("close"))
        vwap = self._safe_float(row.get("vwap"))
        slope = self._safe_float(row.get("vwap_slope"))
        efficiency = self._safe_float(row.get("price_efficiency"))
        flip_count = self._safe_float(row.get("vwap_flip_count"))
        if atr is None or atr <= 0:
            slope_atr = None
            distance_atr = None
        else:
            slope_atr = None if slope is None else abs(slope) / atr
            distance_atr = None if close is None or vwap is None else abs(close - vwap) / atr
        return efficiency, slope_atr, distance_atr, flip_count

    def _precision_blocks(self, row, direction, setup_type):
        """Second-stage precision veto for low-quality VWAP environments.

        This is intentionally asymmetric: ordinary continuation/bounce setups
        need a clean trend environment, while reclaim/failed-cross setups retain
        a lower threshold because they are reversal setups.
        """
        if not self.config.use_precision_regime:
            return False

        efficiency, slope_atr, distance_atr, flip_count = self._precision_metrics(row)
        adx = self._safe_float(row.get("adx"))
        close = self._safe_float(row.get("close"))
        vwap = self._safe_float(row.get("vwap"))
        atr = self._safe_float(row.get("atr"))

        # Warm-up safety: never reject simply because a derived metric has not
        # accumulated enough history yet.
        if efficiency is not None:
            min_eff = (
                float(self.config.precision_reversal_min_efficiency)
                if setup_type in ("FAILED_CROSS", "VWAP_RECLAIM", "VWAP_INTERACTION")
                else float(self.config.precision_min_efficiency)
            )
            # Strong trend strength can compensate for a lower efficiency ratio.
            strong_trend = adx is not None and adx >= float(self.config.quality_strong_adx)
            if efficiency < min_eff and not strong_trend:
                return True

        # A continuation should not be entered when VWAP is effectively flat.
        # Reversal families are allowed to work against the existing slope.
        if setup_type in ("CLOSE_CROSS", "VWAP_BOUNCE") and slope_atr is not None:
            if slope_atr < float(self.config.precision_min_slope_atr):
                strong_trend = adx is not None and adx >= float(self.config.quality_strong_adx)
                if not strong_trend:
                    return True

        # Too many VWAP flips in a short window is a direct chop signature.
        if flip_count is not None and flip_count > float(self.config.precision_max_flip_count):
            strong_move = (
                atr is not None and atr > 0 and close is not None and vwap is not None
                and abs(close - vwap) >= atr * 0.30
            )
            if not strong_move:
                return True

        # Avoid chasing a continuation after price has become excessively
        # extended from VWAP. Reversal setups are intentionally exempt.
        if setup_type in ("CLOSE_CROSS", "VWAP_BOUNCE") and distance_atr is not None:
            if distance_atr > float(self.config.precision_max_setup_distance_atr):
                return True

        # Opening-window regime veto: if the first several candles repeatedly
        # cross VWAP while price has poor directional efficiency, wait for the
        # market to prove a direction rather than repeatedly arming both sides.
        opening_bars = int(row.get("session_bars", 0) or 0)
        opening_flips = int(row.get("session_flip_count", 0) or 0)
        opening_eff = self._safe_float(row.get("session_efficiency"))
        if opening_bars >= int(self.config.precision_opening_bars):
            if (
                opening_flips > int(self.config.precision_opening_max_flips)
                and opening_eff is not None
                and opening_eff < float(self.config.precision_opening_min_efficiency)
            ):
                return True

        # Late-session entries have historically been the weakest part of this
        # dataset. Permit an unusually strong move through the cutoff, but do
        # not keep taking ordinary marginal VWAP setups late in the day.
        dt = row.get("datetime")
        if dt is not None:
            try:
                ts = pd.Timestamp(dt)
                minute = ts.hour * 60 + ts.minute
                cutoff = int(self.config.precision_late_entry_hour) * 60 + int(self.config.precision_late_entry_minute)
                if minute >= cutoff:
                    strong_late = (
                        self.config.precision_allow_strong_late_move
                        and adx is not None and adx >= float(self.config.quality_strong_adx)
                        and efficiency is not None and efficiency >= 0.50
                    )
                    if not strong_late and setup_type in ("CLOSE_CROSS", "VWAP_BOUNCE"):
                        return True
            except Exception:
                pass
    ...
    def _quality_blocks(self, row, direction, setup_type):
        """Return True only for a clearly poor setup."""
        if not self.config.use_setup_quality:
            return False

        score = self._setup_quality_score(row, direction, setup_type)
        if score is None:
            # Indicator warm-up must never make the strategy silently stop.
            return False

        thresholds = {
            "CLOSE_CROSS": int(self.config.quality_close_cross_min),
            "VWAP_RECLAIM": int(self.config.quality_reclaim_min),
            "VWAP_BOUNCE": int(self.config.quality_bounce_min),
            "VWAP_INTERACTION": int(self.config.quality_interaction_min),
            "FAILED_CROSS": int(self.config.quality_failed_cross_min),
        }
        if score < thresholds.get(setup_type, 3):
            return True

        # A strong, clearly established opposite trend is the most dangerous
        # case for an ordinary continuation/bounce. Reversal setups are exempt
        # because their purpose is to catch that transition.
        if (
            self.config.block_strong_countertrend
            and setup_type in ("CLOSE_CROSS", "VWAP_BOUNCE")
        ):
            adx = self._safe_float(row.get("adx"))
            slope = self._safe_float(row.get("vwap_slope"))
            if adx is not None and adx >= float(self.config.quality_strong_adx) and slope is not None:
                if direction == 1 and slope < 0:
                    return True
                if direction == -1 and slope > 0:
                    return True

        if self._precision_blocks(row, direction, setup_type):
            return True

        return False

    def setup_quality(self, row, direction, setup_type):
        return self._setup_quality_score(row, direction, setup_type)
            ...
            if self.cross_bar is not None:
                ...
                if self.config.allow_failed_cross and bars == 1:
                    ...
                    if self.cross_direction == 1 and close < v:
                        self._clear_setup()
                        if not self._quality_blocks(row, -1, "FAILED_CROSS"):
    ...
    def prepare(df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Causal price-efficiency ratio: net directional movement divided by
        # total absolute movement. High values = clean trend, low values = chop.
        eff_len = 8
        net_move = out["close"] - out["close"].shift(eff_len)
        path = out["close"].diff().abs().rolling(eff_len, min_periods=eff_len).sum()
        out["price_efficiency"] = net_move.abs() / path.replace(0, pd.NA)

        # Count completed VWAP side changes over the same short window.
        relation = pd.Series(0, index=out.index, dtype="int64")
        relation[out["close"] > out["vwap"]] = 1
        relation[out["close"] < out["vwap"]] = -1
        side_change = ((relation != relation.shift(1)) & relation.ne(0) & relation.shift(1).ne(0)).astype(int)
        out["vwap_flip_count"] = side_change.groupby(out["date"]).rolling(8, min_periods=2).sum().reset_index(level=0, drop=True)

        # Session-opening regime statistics. These reset every trading day and
        # therefore cannot leak information from the previous session.
        day_pos = out.groupby("date").cumcount() + 1
        out["session_bars"] = day_pos
        out["session_flip_count"] = side_change.groupby(out["date"]).cumsum()
        session_path = out.groupby("date")["close"].diff().abs().groupby(out["date"]).cumsum()
        session_first_close = out.groupby("date")["close"].transform("first")
        out["session_efficiency"] = (out["close"] - session_first_close).abs() / session_path.replace(0, pd.NA)
