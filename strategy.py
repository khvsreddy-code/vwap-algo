...
class StrategyConfig:
    ...
    # v9.4.55 quality layer — intentionally based on v9.4.51.
    # Only activates in clearly directional markets; it does not impose a
    # daily trade limit and does not replace the existing VWAP setup families.
    avoid_countertrend: bool = True
    strong_trend_adx: float = 25.0

    # Once a setup has been armed, a later close decisively back through VWAP
    # invalidates that old setup. This prevents an old cross from firing several
    # candles later after the market has already reversed.
...
class VwapConfirmationEngine:
    ...
    def _strong_trend_allows(self, row, direction):
        """Allow normal VWAP entries unless a strong trend clearly opposes them."""
        ...
        # If slope is available, strong trend direction must agree with setup.
        ...
        # The completed setup candle must finish on the intended side.
    ...
    def _detect_vwap_cross(self, row, first_session_candle=False):
        ...
        try:
            ...
            if long_cross:
                if (not self._chop_blocks(row, 1)
                        and self._market_regime(row) != "RANGE"
                        and self._strong_trend_allows(row, 1)):
            ...
            if bullish_reclaim and not self._chop_blocks(row, 1) and self._market_regime(row) != "RANGE" and self._strong_trend_allows(row, 1):
                return (1, "VWAP_INTERACTION" if rounded else "VWAP_RECLAIM", "HIGH")
            if bearish_reclaim and not self._chop_blocks(row, -1) and self._market_regime(row) != "RANGE" and self._strong_trend_allows(row, -1):
                return (-1, "VWAP_INTERACTION" if rounded else "VWAP_RECLAIM", "HIGH")
            if bullish_bounce and not self._chop_blocks(row, 1) and self._market_regime(row) != "RANGE" and self._strong_trend_allows(row, 1):
                return (1, "VWAP_BOUNCE", "MEDIUM")
            if bearish_bounce and not self._chop_blocks(row, -1) and self._market_regime(row) != "RANGE" and self._strong_trend_allows(row, -1):
                return (-1, "VWAP_BOUNCE", "MEDIUM")
