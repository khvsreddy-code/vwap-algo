...
class VwapConfirmationEngine:
    ...
    def seed_from_history(self, df: pd.DataFrame):
        """
        Rebuild the exact pending-setup state from already-closed history.

        Historical seeding must replay the *confirmation/expiry* rules as well
        as VWAP crosses. Otherwise a setup that actually confirmed or expired
        before the live session could remain armed and fire a false live entry.
        Seeding never emits a live order/signal.
        """
        self.cross_price = None
        self.cross_bar = None
        self.cross_direction = 0
        ...
        for _, row in df.reset_index(drop=True).iterrows():
            ...
            # First consume an already-armed setup.  The historical replay
            # must discard a setup that would have confirmed, because that
            # entry already happened before the live engine started.
            if self.cross_bar is not None:
                bars = self.bar_index - self.cross_bar
                if 1 <= bars <= self.config.confirmation_bars:
                    confirmed = (
                        self.cross_direction == 1 and high >= float(self.confirmation_level)
                    ) or (
                        self.cross_direction == -1 and low <= float(self.confirmation_level)
                    )
                    if confirmed:
                        self._clear_setup()
                        continue
            ...
            # If a setup is still pending, it owns the confirmation window;
            # do not replace it with a later VWAP relationship.
            if self.cross_bar is not None:
                continue
        ...
        # Preserve the last closed candle's date so the first live tick/candle
        # on the same session does not erase a valid pending setup.
    ...
    def process_live_tick(self, ltp, timestamp=None):
        ...
        # A pending setup never crosses a trading-session boundary.  This check
        # is required here because live ticks arrive before the first new
        # session candle closes; waiting for process_closed_candle() would allow
        # a previous day's trigger level to fire on the next day's first tick.
