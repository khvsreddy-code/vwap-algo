...
class StrategyConfig:
    ...
    # Strong-trend alignment: in a clearly directional market, avoid taking
    # counter-trend VWAP signals. This is a quality filter, not a trade quota.
    avoid_countertrend: bool = True
    strong_trend_adx: float = 25.0

    # Range mode is selective rather than a blanket "no trades" switch. Clear
    # crosses are suppressed in chop, while genuine VWAP rejection/reclaim
    # setups can still participate.
    allow_range_rejections: bool = True

    # Minimum wick fraction for the lighter VWAP-touch rejection detector.
    # This catches rounded candles that test VWAP without fully straddling it.
    rejection_wick_fraction: float = 0.25
...
class VwapConfirmationEngine:
    ...
    def _setup_allowed(self, row, direction, setup_type):
        """Apply regime-aware quality gates without imposing a trade quota.

        The filter has three important behaviors:
        * clear range/chop blocks ordinary close-to-close crosses;
        * range can still trade genuine VWAP rejection/reclaim structures;
        * strong directional trends reject counter-trend CE/PE signals.
        """
        regime = self._market_regime(row)

        if regime == "RANGE":
            if not self.config.allow_range_rejections:
                return False
            return setup_type in {"VWAP_BOUNCE", "VWAP_REJECTION", "VWAP_RECLAIM", "FAILED_CROSS"}

        if not self.config.avoid_countertrend:
            return True

        adx = self._safe_float(row.get("adx"))
        slope = self._safe_float(row.get("vwap_slope"))
        if adx is None or adx < float(self.config.strong_trend_adx):
            return True

        # Only enforce alignment when the trend direction itself is clear.
    ...
    def _detect_vwap_cross(self, row, first_session_candle=False):
        ...
        try:
            ...
            if long_cross:
                if not self._chop_blocks(row, 1) and self._setup_allowed(row, 1, "CLOSE_CROSS"):
            ...
            # Lighter touch/rejection detector. A rounded candle may test VWAP
            # with a wick and finish back on the trend side without crossing the
            # entire VWAP band. This is deliberately stricter than a raw "touch":
            # the candle must close back on the same side and show a meaningful
            # rejection wick.
            ...
            bullish_rejection = (
                ...
                and high >= vwap
                and close > vwap + tol
                and lower_wick >= candle_range * float(self.config.rejection_wick_fraction)
            )
            bearish_rejection = (
                self.config.allow_vwap_bounce
                and prev_close_f is not None and prev_vwap_f is not None
                ...
                and low <= vwap
                and close < vwap - tol
                and upper_wick >= candle_range * float(self.config.rejection_wick_fraction)
            )

            if bullish_rejection and not self._chop_blocks(row, 1) and self._setup_allowed(row, 1, "VWAP_REJECTION"):
                return (1, "VWAP_REJECTION", "MEDIUM")
            if bearish_rejection and not self._chop_blocks(row, -1) and self._setup_allowed(row, -1, "VWAP_REJECTION"):
                return (-1, "VWAP_REJECTION", "MEDIUM")
            ...
            if bullish_reclaim and not self._chop_blocks(row, 1) and self._setup_allowed(row, 1, "VWAP_RECLAIM"):
                return (1, "VWAP_INTERACTION" if rounded else "VWAP_RECLAIM", "HIGH")
            if bearish_reclaim and not self._chop_blocks(row, -1) and self._setup_allowed(row, -1, "VWAP_RECLAIM"):
                return (-1, "VWAP_INTERACTION" if rounded else "VWAP_RECLAIM", "HIGH")
            if bullish_bounce and not self._chop_blocks(row, 1) and self._setup_allowed(row, 1, "VWAP_BOUNCE"):
                return (1, "VWAP_BOUNCE", "MEDIUM")
            if bearish_bounce and not self._chop_blocks(row, -1) and self._setup_allowed(row, -1, "VWAP_BOUNCE"):
                return (-1, "VWAP_BOUNCE", "MEDIUM")
