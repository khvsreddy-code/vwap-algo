...
V10 — persistent DOM chart + true incremental updates.
...
_CHART_HTML = """
<div id="vwap-live-chart" style="width:100%;height:620px;min-height:620px;display:block;"></div>
"""
...
export default function(component) {
  ...
  const requestedHeight = Math.max(620, Number(data.height || 620));
  // Streamlit can briefly mount the component with a zero-height parent while
  // a page/fragment is settling. Never let that collapse the chart canvas.
  if (root) {
  ...
  function init() {
    ...
    state.resizeObserver = new ResizeObserver(() => {
      ...
      const h = Math.max(requestedHeight, host.clientHeight || 0);
      if (w > 10 && h > 10) {
