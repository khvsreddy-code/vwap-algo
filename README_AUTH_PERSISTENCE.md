...
## v9.4.18 no-rerun-loop fix
Removed the browser-side auth components because their setStateValue calls
could trigger continuous reruns. Auth details are now persisted by the
server-side OAuth state record before opening FYERS. Open Auth Web is a plain
same-tab anchor.
