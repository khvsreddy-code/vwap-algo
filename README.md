...
2. Use the prefilled Auth Redirect URL and click **Create login link**.
3. Click **Open FYERS Login**. It stays in the same browser tab.
4. Complete the normal FYERS login/authorization.
5. FYERS returns to `?page=auth&auth_code=...&state=...`.
6. The app captures the one-time code, restores the saved App ID/Secret/Redirect URI, exchanges the code for today's access token, and shows the code on the Auth page.
7. Click **Back to Terminal**. The token and credentials remain in the Streamlit session and the Terminal automatically connects.

Important: the Auth page never redirects to itself. The callback parameters are consumed once and then replaced with `?page=auth`, preventing the `share.streamlit.io redirected you too many times` loop.
