...
### V9.4.57 — long-option-only entry logic
...
### V9.4.57 — candle rendering reliability
...
### V9.4.57 — candle pipeline repair

- History retrieval now retries the configured window, recent sessions, and epoch range.
- Non-futures symbols use the non-continuous history flag.
- The live chart uses cached REST history if the background engine has not populated its dataframe yet.
- Empty history is no longer silently presented as a blank chart; the UI reports the History API error.
