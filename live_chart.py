...
export default function(component) {
  ...
  const state = host.__vwapState || {
    ...
    lastPayload: null,
    resizeObserver: null,
    // V2 data updates can arrive faster than gzip decompression completes.
    // Ignore an older async payload if a newer rerun has already arrived.
    payloadSeq: 0,
  };
  ...
  async function decodePayload(input) {
    if (!input || typeof input !== "object") return {};
    if (!input.packed) return input;

    // V2 component data travels through the Streamlit bidi channel. A full
    // month of 5-minute candles can be large enough to make that transport
    // fragile in some browser/runtime combinations. The Python side therefore
    // sends one compact gzip+base64 string. Chrome/Edge expose the native
    // DecompressionStream API, so no extra JS dependency is required.
    try {
      if (typeof DecompressionStream === "undefined") {
        throw new Error("Browser does not support DecompressionStream");
      }
      const binary = atob(String(input.packed));
      ...
      // A transient bidi/decompression failure must NEVER replace a healthy
      // chart with an empty payload. The old behaviour caused the whole chart
      // to blink/disappear and then reappear on the next fragment rerun.
      console.error("VWAP chart payload decode failed; keeping last chart state:", e);
      return null;
    }
  }
  ...
  async function applyPayload(p) {
    ...
    const decoded = await decodePayload(p);
...
def make_payload(
    ...
    """Build a compact JSON-safe chart payload.

    Streamlit Custom Components V2 uses a bidirectional transport for `data`.
    Sending thousands of 5-minute candle objects directly can exceed the
    practical payload size of some deployed browser/runtime combinations and
    surface as `BidiComponent Error: Unexpected end of input`. Keep the full
    month in Python, but gzip the browser payload so the same full history is
    delivered reliably.
    """
    if df is None or df.empty:
        ...
        # Live charts keep a small rolling window for performance. Historical,
        # replay and full-backtest views pass max_candles=None so the complete
        # currently-loaded month/window is sent to the browser.
    ...
    raw = json.dumps(core, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
