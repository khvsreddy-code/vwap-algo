...
export default function(component) {
  ...
  function syncSeries(series, incoming, stateKey) {
    // Historical/backtest/replay payloads can change the visible window while
    // keeping the same first candle.  Incremental update() is only safe for a
    // live rolling feed; for replay it can leave the browser series out of
    // sync with the marker timestamps.  Re-send the complete payload so every
    // marker timestamp is guaranteed to belong to the candle series currently
    // displayed.
  ...
  function applyPayload(p) {
    ...
    // Lightweight Charts 4.x uses the series-marker plugin API. Older builds
    // exposed series.setMarkers(), so support both without breaking either.
    ...
    try {
      ...
      } else if (typeof state.candles.setMarkers === "function") {
        state.candles.setMarkers(markerData);
      }
    } catch (e) {
      // Never let marker rendering break the candle/chart itself.
      try {
        if (typeof state.candles.setMarkers === "function") state.candles.setMarkers(markerData);
      } catch (_) {}
    }
